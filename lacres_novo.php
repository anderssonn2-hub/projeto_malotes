...
// v2.9.5: coluna snapshot_grade (LONGTEXT) em ciDespachos guarda o ESTADO EXATO da grade
// Correios no momento do salvamento: linhas adicionadas manualmente (com numero real + nome)
// e os lacres EXATAMENTE como digitados (faixa com hifen, ex.: "53755-53756"). Permite
// restaurar o oficio completo ao filtrar pela data depois (continuar em outro dia).
// DDL ANTES de qualquer transacao (em MySQL DDL faz commit implicito). Fail-open: se o
// banco nao permitir o ALTER, o sistema segue (save/restore de snapshot ficam inativos).
try {
    $colsSnapGrade = $pdo_controle->query("SHOW COLUMNS FROM ciDespachos LIKE 'snapshot_grade'")->fetchAll();
    if (count($colsSnapGrade) === 0) {
        $pdo_controle->exec("ALTER TABLE ciDespachos ADD COLUMN snapshot_grade LONGTEXT NULL");
    }
} catch (Exception $eSnapCol) {
    // segue sem a coluna (snapshot indisponivel)
}
...
// v2.9.8: filtro de grupo (Correios x Poupa Tempo) para acelerar o carregamento da
// grade. Carrega SOMENTE os postos do grupo escolhido. Persistido em sessao para
// sobreviver aos POSTs de salvamento/edicao (igual ao filtro de datas).
if (isset($_GET['filtro_grupo'])) {
    $_SESSION['filtro_grupo'] = ($_GET['filtro_grupo'] === 'correios' || $_GET['filtro_grupo'] === 'poupatempo') ? $_GET['filtro_grupo'] : '';
}
$filtro_grupo = isset($_SESSION['filtro_grupo']) ? $_SESSION['filtro_grupo'] : '';
...
if (isset($_POST['acao']) && $_POST['acao'] === 'verificar_etiqueta') {
    ...
    // Resposta: alem do posto, informa se o display esta cadastrado (#6),
    // se esta em transito (ultimo movimento tipo=1, #5) e se consta no
    // inventario fisico (presente => nao prende para registrar retorno).
    $resp = array(
        'ok' => true, 'posto' => null, 'cep' => $cep_etiq, 'seq' => $seq_etiq,
        'leitura' => $leitura, 'cadastrado' => false,
        'em_transito' => false, 'presente_inventario' => false
    );
    try {
        // Match EXATO (cep+sequencial) define se o display ja esta cadastrado (#6).
        ...
        if ($row) {
            $resp['cadastrado'] = true;
            if (isset($row['posto']) && trim($row['posto']) !== '') {
                $resp['posto'] = trim($row['posto']);
            }
        } else {
            // Fallback só por CEP: serve apenas para SUGERIR o posto na janela
            // de cadastro; NAO marca como cadastrado (a etiqueta exata ainda nao existe).
            ...
            if ($row2 && isset($row2['posto']) && trim($row2['posto']) !== '') {
                $resp['posto'] = trim($row2['posto']);
            }
        }
    } catch (Exception $e) {
    ...
    // Ultimo movimento em ciMalotes: em transito se for tipo=1 (saiu e nao voltou).
    try {
        $stM = $pdo_controle->prepare('SELECT tipo FROM ciMalotes WHERE leitura=? ORDER BY id DESC LIMIT 1');
        $stM->execute(array($leitura));
        $rM = $stM->fetch(PDO::FETCH_ASSOC);
        if ($rM && (int)$rM['tipo'] === 1) { $resp['em_transito'] = true; }
    } catch (Exception $e) { /* tolera */ }
    // Inventario fisico: se presente, o display voltou (mesmo sem tipo=2).
    try {
        $stI = $pdo_controle->prepare('SELECT 1 FROM ciInventarioDisplays WHERE leitura=? LIMIT 1');
        $stI->execute(array($leitura));
        if ($stI->fetch()) { $resp['presente_inventario'] = true; }
    } catch (Exception $e) { /* tabela pode nao existir */ }
    echo json_encode($resp);
    exit;
}

// Validacao em LOTE antes de gravar o oficio Correios: recebe leitura[] (+ posto[]
// da linha) e devolve quais etiquetas NAO estao cadastradas em cadastroMalotes
// (match EXATO cep+sequencial). Usado para avisar e oferecer cadastro antes do save.
if (isset($_POST['acao']) && $_POST['acao'] === 'verificar_etiquetas_lote') {
    header('Content-Type: application/json; charset=utf-8');
    $leituras = (isset($_POST['leitura']) && is_array($_POST['leitura'])) ? $_POST['leitura'] : array();
    $postos   = (isset($_POST['posto'])   && is_array($_POST['posto']))   ? $_POST['posto']   : array();
    $naoCadastradas = array();
    try {
        $stExato = $pdo_controle->prepare('SELECT COUNT(*) FROM cadastroMalotes WHERE cep=? AND sequencial=?');
        $stCep   = $pdo_controle->prepare('SELECT posto FROM cadastroMalotes WHERE cep=? ORDER BY id DESC LIMIT 1');
        $vistos = array();
        $idx = -1;
        foreach ($leituras as $lr) {
            $idx++;
            $dig = preg_replace('/\D+/', '', (string)$lr);
            if (strlen($dig) !== 35) continue;
            if (isset($vistos[$dig])) continue;
            $vistos[$dig] = true;
            $cep = substr($dig, 0, 8);
            $seq = substr($dig, -5);
            $stExato->execute(array($cep, $seq));
            if ((int)$stExato->fetchColumn() > 0) continue; // ja cadastrada (cep+seq)
            // Posto sugerido por CEP (apenas sugestao para a janela de cadastro)
            $postoSug = '';
            try {
                $stCep->execute(array($cep));
                $rc = $stCep->fetch(PDO::FETCH_ASSOC);
                if ($rc && isset($rc['posto'])) { $postoSug = trim((string)$rc['posto']); }
            } catch (Exception $e2) { /* tolera */ }
            $postoLinha = isset($postos[$idx]) ? preg_replace('/\s+/', '', (string)$postos[$idx]) : '';
            $naoCadastradas[] = array(
                'leitura' => $dig, 'cep' => $cep, 'seq' => $seq,
                'posto_linha' => $postoLinha, 'posto_sugerido' => $postoSug
            );
        }
    } catch (Exception $e) {
        echo json_encode(array('ok' => false, 'msg' => 'Erro BD'));
    ...
    echo json_encode(array('ok' => true, 'nao_cadastradas' => $naoCadastradas));
    exit;
}

// #6: cadastra um display que ainda nao esta em cadastroMalotes (janela na tela).
if (isset($_POST['acao']) && $_POST['acao'] === 'cadastrar_display') {
    header('Content-Type: application/json; charset=utf-8');
    $leitura = preg_replace('/\D+/', '', (string)(isset($_POST['leitura']) ? $_POST['leitura'] : ''));
    $posto   = preg_replace('/\s+/', '', (string)(isset($_POST['posto']) ? $_POST['posto'] : ''));
    if (strlen($leitura) !== 35 || $posto === '') {
        echo json_encode(array('ok' => false, 'msg' => 'Dados invalidos')); exit;
    }
    $cep = substr($leitura, 0, 8); $seq = substr($leitura, -5);
    try {
        $chk = $pdo_controle->prepare('SELECT COUNT(*) FROM cadastroMalotes WHERE leitura=?');
        $chk->execute(array($leitura));
        if ((int)$chk->fetchColumn() > 0) {
            echo json_encode(array('ok' => true, 'jaExistia' => true, 'msg' => 'Display ja cadastrado')); exit;
        }
        $ins = $pdo_controle->prepare(
            'INSERT INTO cadastroMalotes (leitura, cep, sequencial, posto) VALUES (?, ?, ?, ?)'
        );
...
// #5: registra o retorno (tipo=2) de um display em transito antes de continuar,
// mantendo a consistencia das tabelas de displays. Espelha devolucao_etiquetas.php.
if (isset($_POST['acao']) && $_POST['acao'] === 'registrar_retorno') {
    header('Content-Type: application/json; charset=utf-8');
    $leitura = preg_replace('/\D+/', '', (string)(isset($_POST['leitura']) ? $_POST['leitura'] : ''));
    $responsavel = trim((string)(isset($_POST['responsavel']) ? $_POST['responsavel'] : ''));
    if (strlen($leitura) !== 35) { echo json_encode(array('ok' => false, 'msg' => 'Etiqueta invalida')); exit; }
    if ($responsavel === '') { $responsavel = isset($_SESSION['usuario']) ? $_SESSION['usuario'] : 'conferencia'; }
    $cep = substr($leitura, 0, 8); $seq = substr($leitura, -5);
    try {
        $c2 = $pdo_controle->prepare('SELECT COUNT(*) FROM ciMalotes WHERE leitura=? AND tipo=2 AND DATE(data)=CURDATE()');
        $c2->execute(array($leitura));
        if ((int)$c2->fetchColumn() > 0) {
            echo json_encode(array('ok' => true, 'jaExistia' => true, 'msg' => 'Retorno ja registrado hoje')); exit;
        }
        // So registra retorno se o display realmente esta em transito:
        // ultimo movimento deve ser tipo=1 (saiu e nao voltou). Evita gravar
        // tipo=2 fora de sequencia em chamadas diretas/forjadas ao handler.
        $cMov = $pdo_controle->prepare('SELECT tipo FROM ciMalotes WHERE leitura=? ORDER BY id DESC LIMIT 1');
        $cMov->execute(array($leitura));
        $rMov = $cMov->fetch(PDO::FETCH_ASSOC);
        if (!$rMov || (int)$rMov['tipo'] !== 1) {
            echo json_encode(array('ok' => false, 'msg' => 'Display nao esta em transito (sem saida pendente)')); exit;
        }
        // Resolve posto pelo cadastro (cep+seq, fallback cep).
        $posto = null;
        try {
            $stp = $pdo_controle->prepare('SELECT posto FROM cadastroMalotes WHERE cep=? AND sequencial=? ORDER BY id DESC LIMIT 1');
            $stp->execute(array($cep, $seq));
            $rp = $stp->fetch(PDO::FETCH_ASSOC);
            if (!$rp) {
                $stp2 = $pdo_controle->prepare('SELECT posto FROM cadastroMalotes WHERE cep=? ORDER BY id DESC LIMIT 1');
                $stp2->execute(array($cep));
                $rp = $stp2->fetch(PDO::FETCH_ASSOC);
            }
            if ($rp && isset($rp['posto'])) { $posto = $rp['posto']; }
        } catch (Exception $e) { /* tolera */ }
        $ins = $pdo_controle->prepare(
            'INSERT INTO ciMalotes (leitura, data, observacao, login, tipo, cep, sequencial, posto) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
        );
        $ins->execute(array($leitura, date('Y-m-d'), 'Retorno via tela de lacres', $responsavel, 2, $cep, $seq, $posto));
        echo json_encode(array('ok' => true));
    } catch (Exception $e) {
        echo json_encode(array('ok' => false, 'msg' => $e->getMessage()));
    }
...
// Adiantamento de lote: o operador le o codigo de barras do lote (19 digitos) produzido
// HOJE que seguira junto com a producao deste oficio, informa a data de producao e o
// executor. Grava em ciDespachoAdiantado. No dia seguinte, ao filtrar a producao daquela
// data, a linha do posto reaparece em vermelho ("JA DESPACHADO") para ser removida.
if (isset($_POST['acao']) && $_POST['acao'] === 'adiantar_lote') {
    header('Content-Type: application/json; charset=utf-8');
    $cod = preg_replace('/\D+/', '', (string)(isset($_POST['codbar']) ? $_POST['codbar'] : ''));
    $dataProd = trim((string)(isset($_POST['data_producao']) ? $_POST['data_producao'] : ''));
    $executor = trim((string)(isset($_POST['executor']) ? $_POST['executor'] : ''));
    if (strlen($cod) < 19) { echo json_encode(array('ok' => false, 'msg' => 'Codigo do lote invalido (precisa de 19 digitos).')); exit; }
    $cod = substr($cod, -19); // ultimos 19 digitos validos (igual a conferencia)
    $lote  = substr($cod, 0, 8);
    $posto = substr($cod, 11, 3);
    $qtd   = (int)substr($cod, 14, 5);
    $postoPad = str_pad(preg_replace('/\D/', '', $posto), 3, '0', STR_PAD_LEFT);
    if ($postoPad === '' || $postoPad === '000') { echo json_encode(array('ok' => false, 'msg' => 'Posto invalido no codigo do lote.')); exit; }
    if ($executor === '') { echo json_encode(array('ok' => false, 'msg' => 'Informe o nome do executor.')); exit; }
    // data de producao -> Y-m-d (aceita yyyy-mm-dd do input date ou dd-mm-yyyy / dd/mm/yyyy)
    $dpYmd = null;
    if ($dataProd !== '') {
        $dt = DateTime::createFromFormat('Y-m-d', $dataProd);
        if (!$dt) { $dt = DateTime::createFromFormat('d-m-Y', str_replace('/', '-', $dataProd)); }
        if ($dt) { $dpYmd = $dt->format('Y-m-d'); }
    }
    if ($dpYmd === null) { echo json_encode(array('ok' => false, 'msg' => 'Informe uma data de producao valida.')); exit; }
    try {
        $pdo_controle->exec("CREATE TABLE IF NOT EXISTS ciDespachoAdiantado (
        ...
        // descobre colunas existentes (tabela pode ter schema "rico" criado manualmente)
        $cols = array();
        try {
            $sc = $pdo_controle->query("SHOW COLUMNS FROM ciDespachoAdiantado");
            if ($sc) { while ($c = $sc->fetch(PDO::FETCH_ASSOC)) { $cols[strtolower($c['Field'])] = true; } }
        } catch (Exception $e) { /* tolera */ }
        // dedupe: remove um adiantamento anterior identico (mesmo posto+lote+data_producao)
        try {
            $pdo_controle->prepare("DELETE FROM ciDespachoAdiantado WHERE posto=? AND lote=? AND data_producao=?")
                         ->execute(array($postoPad, $lote, $dpYmd));
        } catch (Exception $e) { /* tolera */ }
        $fields = array('posto', 'lote', 'data_producao', 'data_despacho');
        $place  = array('?', '?', '?', 'CURDATE()');
        $vals   = array($postoPad, $lote, $dpYmd);
        $obs    = 'Executor: ' . $executor . ' | Qtd: ' . $qtd;
        if (isset($cols['observacao'])) { $fields[] = 'observacao'; $place[] = '?'; $vals[] = substr($obs, 0, 255); }
        if (isset($cols['quantidade'])) { $fields[] = 'quantidade'; $place[] = '?'; $vals[] = $qtd; }
        $sql = "INSERT INTO ciDespachoAdiantado (" . implode(',', $fields) . ") VALUES (" . implode(',', $place) . ")";
        $st = $pdo_controle->prepare($sql);
        $st->execute($vals);
        echo json_encode(array('ok' => true, 'posto' => $postoPad, 'lote' => $lote, 'qtd' => $qtd));
    } catch (Exception $e) {
        echo json_encode(array('ok' => false, 'msg' => $e->getMessage()));
    }
    exit;
...
if (isset($_POST['acao']) && $_POST['acao'] === 'salvar_oficio_correios') {
    try {
        ...
        // v2.0.3 #7: garantir a tabela FORA da transacao (DDL faz commit implicito no MySQL).
        // Schema ALINHADO a tabela ja existente no banco do usuario (id PK + colunas extras);
        // o recurso so usa 'posto', preenchendo 'data_despacho' (NOT NULL) com a data do dia.
        try {
            $pdo_controle->exec("CREATE TABLE IF NOT EXISTS ciDespachoAdiantado (
                id INT(11) NOT NULL AUTO_INCREMENT,
                posto VARCHAR(10) NOT NULL,
        ...
        $stmtLotes = $pdo_controle->prepare($sqlLotes);
        $stmtLotes->execute($datasSql);
        // Lotes do filtro de datas (fonte unica). v2.0.3 #2 (estante) REMOVIDO: nao se anexa
        // mais lote de posto fora do filtro so porque o display voltou.
        $lotesParaProcessar = $stmtLotes->fetchAll(PDO::FETCH_ASSOC);
        ...
        // v2.9.5: Persistir o SNAPSHOT EXATO da grade no despacho (restauravel por data
        // depois). Guarda numero real + nome das linhas manuais e os lacres com hifen
        // exatamente como digitados. Os writes numericos abaixo (etiquetaiipr etc.) seguem
        // inalterados (a continuidade/MAX de lacre depende deles). Coluna pode nao existir
        // em bancos antigos -> fail-open (nao quebra o salvamento).
        if (isset($id_desp) && $id_desp && isset($_POST['snapshot_oficio'])
            && trim((string)$_POST['snapshot_oficio']) !== '' && !empty($snapshot)) {
            try {
                $stSnapSave = $pdo_controle->prepare("UPDATE ciDespachos SET snapshot_grade = ? WHERE id = ?");
                $stSnapSave->execute(array((string)$_POST['snapshot_oficio'], (int)$id_desp));
            } catch (Exception $eSnapSave) {
                // banco antigo sem a coluna snapshot_grade: segue normalmente
            }
        }
        ...
        foreach ($lotesParaProcessar as $l) {
            ...
            if (is_array($lacreDetalhado)) {
                ...
                // v2.2.1: a etiqueta editada na grade (snapshot/form) tem prioridade
                // sobre a etiqueta da conferencia (chips). A tabela conferencia_pacotes_lacres
                // e somente leitura neste fluxo e mantem o valor da conferencia original;
                // se o operador troca a etiqueta no oficio e regrava, o chip NAO pode
                // sobrescrever o valor novo. So usa o chip quando a grade nao informou etiqueta.
                if ((!isset($etiquetaCorreios_lote) || trim((string)$etiquetaCorreios_lote) === '')
                    && isset($lacreDetalhado['etiqueta_correios']) && trim((string)$lacreDetalhado['etiqueta_correios']) !== '') {
        ...
        if (!empty($etiquetasOficio)) {
            ...
            // 3) Resolve o posto de cada display do oficio (cadastro cep+seq >
            //    cadastro cep > posto do proprio oficio) ANTES de gravar.
            $registros = array();          // eti_digits => array('cep','seq','posto')
            $postosResolvidos = array();   // posto numerico => true (escopo da limpeza)
            foreach ($etiquetasOficio as $eti_digits => $infoE) {
                ...
                $registros[$eti_digits] = array('cep' => $cep, 'seq' => $seq, 'posto' => $posto_malote);
                // So postos numericos entram na limpeza (evita mexer no bucket
                // compartilhado 'central', usado por varios oficios no mesmo dia).
                if (preg_match('/^\d+$/', $posto_malote)) {
                    $postosResolvidos[$posto_malote] = true;
                }
            }

            // 3b) SOBRESCREVER em vez de duplicar: ao regravar o mesmo oficio
            //     (ex.: trocar a etiqueta de um posto), remove os displays tipo=1
            //     gravados HOJE para os postos deste oficio cujas etiquetas NAO
            //     estao mais no oficio atual. Assim a etiqueta antiga sai e fica
            //     so a atual (corrige a duplicacao em ciMalotes ao sobrescrever).
            if (!empty($registros) && !empty($postosResolvidos)) {
                try {
                    $etisAtuais  = array_keys($registros);
                    $postosLista = array_keys($postosResolvidos);
                    $phP = implode(',', array_fill(0, count($postosLista), '?'));
                    $phE = implode(',', array_fill(0, count($etisAtuais), '?'));
                    $stDel = $pdo_controle->prepare(
                        'DELETE FROM ciMalotes WHERE tipo = 1 AND DATE(data) = CURDATE()'
                        . ' AND posto IN (' . $phP . ') AND leitura NOT IN (' . $phE . ')'
                    );
                    $stDel->execute(array_merge($postosLista, $etisAtuais));
                } catch (Exception $eDel) { /* tolera: segue sem a limpeza */ }
            }

            // 3c) Duplicatas do dia em UMA consulta (nao regrava o que ja saiu hoje)
            $jaSalvasHoje = array();
            if (!empty($registros)) {
                try {
                    $etisLista = array_keys($registros);
            ...
            foreach ($registros as $eti_digits => $reg) {
                ...
                $stmtMalotes->execute(array(
                    ...
                    $reg['cep'],    // cep
                    $reg['seq'],    // sequencial
                    $reg['posto']   // posto (cadastro quando existe; senao o do oficio)
                ));
                $etiquetas_salvas++;
            }
        }
        
        // Adiantamento: a marcacao deixou de usar checkbox por linha. Agora o operador
        // grava o lote adiantado via botao "Adiantar lote" (handler 'adiantar_lote'),
        // que escreve em ciDespachoAdiantado. Nada a persistir aqui no save do oficio.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['inserir_linha'])) {
        ...
        // Numero real do posto (ex.: 808) usado SO para validar o display bipado.
        // O codigo interno da linha continua sendo o "M..." unico; este campo
        // guarda o numero verdadeiro do posto para a deteccao de display nao
        // disparar falso "display de outro posto" em linhas inseridas manualmente.
        $novo_codigo = isset($_POST['novo_codigo']) ? preg_replace('/\D+/', '', $_POST['novo_codigo']) : '';
        if ($novo_codigo === '' && preg_match('/(\d{2,5})/', $novo_nome, $mNum)) {
            $novo_codigo = $mNum[1];
        }
    ...
    if (isset($_POST['adicionar_manual'])) {
        ...
        if (empty($posto_nome)) {
            ...
            $codigo = 'M' . time() . rand(1000, 9999);
            $posto_real_add = '';
            if (preg_match('/(\d{2,5})/', $posto_nome, $mNumAdd)) {
                $posto_real_add = $mNumAdd[1];
            }
            $_SESSION['postos_manuais'][$codigo] = array(
                'posto_codigo' => $codigo,
                'posto_real' => $posto_real_add,
...
// T4: lista de lotes do periodo aplicado que AINDA NAO foram conferidos
// (conferencia_pacotes.conf <> 's'). Usado para AVISAR (nao bloqueia) ao gerar oficio.
$lotes_nao_conferidos = array();
if (!empty($datas_filtro)) {
    try {
        $datasSqlT4 = array();
        foreach ($datas_filtro as $d) {
            $obj = DateTime::createFromFormat('d-m-Y', $d);
            if ($obj) { $datasSqlT4[] = $obj->format('Y-m-d'); }
        }
        if (!empty($datasSqlT4)) {
            // Confere se a tabela de conferencia existe (tolerante ao schema)
            $temConfTabela = false;
            try {
                $chkConf = $pdo_controle->query("SHOW TABLES LIKE 'conferencia_pacotes'")->fetchAll();
                $temConfTabela = !empty($chkConf);
            } catch (Exception $e) { $temConfTabela = false; }

            $phT4 = implode(',', array_fill(0, count($datasSqlT4), '?'));
            if ($temConfTabela) {
                $sqlT4 = "
                    SELECT LPAD(c.posto,3,'0') AS posto,
                           c.lote AS lote,
                           SUM(COALESCE(c.quantidade,0)) AS quantidade
                    FROM ciPostosCsv c
                    WHERE DATE(c.dataCarga) IN ($phT4)
                      AND NOT EXISTS (
                          SELECT 1 FROM conferencia_pacotes cp
                          WHERE cp.nlote = c.lote AND cp.conf = 's'
                      )
                    GROUP BY LPAD(c.posto,3,'0'), c.lote
                    ORDER BY LPAD(c.posto,3,'0'), c.lote
                ";
            } else {
                // Sem tabela de conferencia: todos os lotes do periodo contam como nao conferidos
                $sqlT4 = "
                    SELECT LPAD(c.posto,3,'0') AS posto,
                           c.lote AS lote,
                           SUM(COALESCE(c.quantidade,0)) AS quantidade
            ...
            $stmtT4 = $pdo_controle->prepare($sqlT4);
            $stmtT4->execute($datasSqlT4);
            while ($rowT4 = $stmtT4->fetch(PDO::FETCH_ASSOC)) {
                $lotes_nao_conferidos[] = array(
                    'posto' => (string)$rowT4['posto'],
                    'lote'  => (string)$rowT4['lote'],
                    'quantidade' => (int)$rowT4['quantidade']
                );
            }
        }
    } catch (Exception $e) {
        // Banco fora (LAN) ou schema diferente: avisar nao deve travar a pagina
        $lotes_nao_conferidos = array();
    }
}

// NOVO: lotes do periodo aplicado que JA participaram de oficio anterior COM lacres
// associados a eles (ciDespachoLotes ligado a ciDespachoItens que tenha lacre_iipr ou
// lacre_correios preenchido). Usado para: (a) pintar as linhas PT em outra cor;
// (b) avisar em janela quando o filtro de datas e aplicado; (c) alertar novamente antes
// de gerar o oficio, evitando duplicidade. SOMENTE LEITURA (nao grava nada).
$lotes_ja_em_oficio  = array(); // lista [posto, lote, quantidade] para a janela de aviso
$postos_ja_em_oficio = array(); // mapa posto(3 dig) => true para pintar/checar a linha
if (!empty($datas_filtro)) {
    try {
        $datasSqlJO = array();
        foreach ($datas_filtro as $d) {
            $obj = DateTime::createFromFormat('d-m-Y', $d);
            if ($obj) { $datasSqlJO[] = $obj->format('Y-m-d'); }
        }
        if (!empty($datasSqlJO)) {
            // Tolerante ao schema: so consulta se as duas tabelas existirem
            $temLotesTab = false; $temItensTab = false;
            try { $rowsLT = $pdo_controle->query("SHOW TABLES LIKE 'ciDespachoLotes'")->fetchAll(); $temLotesTab = !empty($rowsLT); } catch (Exception $e) { $temLotesTab = false; }
            try { $rowsIT = $pdo_controle->query("SHOW TABLES LIKE 'ciDespachoItens'")->fetchAll(); $temItensTab = !empty($rowsIT); } catch (Exception $e) { $temItensTab = false; }
            if ($temLotesTab && $temItensTab) {
                // Excluir APENAS o oficio ATUAL que esta sendo (re)gerado para estas
                // mesmas datas — sobrescrever o proprio oficio NAO e duplicidade.
                // Lotes presentes em OUTRO oficio (datas diferentes, ou copia criada
                // com "modo=novo") continuam sendo sinalizados.
                //
                // Como distinguir o "oficio atual" das copias modo=novo de mesma data:
                // o save canonico (sobrescrever) grava hash_chave = sha1('POUPA TEMPO'|datas_str);
                // ja o modo=novo grava sha1(grupo|datas_str|time|rand), que NUNCA bate com
                // o hash canonico. Entao, entre os despachos com o MESMO conjunto de datas
                // do filtro atual, excluimos so aquele cujo hash_chave == hash canonico
                // (calculado a partir da PROPRIA datas_str armazenada do despacho).
                $excluirIdsJO = array();
                try {
                    $rowsDT = $pdo_controle->query("SHOW TABLES LIKE 'ciDespachos'")->fetchAll(); $temDespTab = !empty($rowsDT);
                    if ($temDespTab) {
                        $setAtual = $datasSqlJO;
                        sort($setAtual);
                        $setAtualKey = implode(',', array_unique($setAtual));
                        $stDesp = $pdo_controle->query("SELECT id, datas_str, hash_chave FROM ciDespachos WHERE grupo = 'POUPA TEMPO'");
                        if ($stDesp) {
                            while ($rd = $stDesp->fetch(PDO::FETCH_ASSOC)) {
                                $ds = isset($rd['datas_str']) ? (string)$rd['datas_str'] : '';
                                ...
                                sort($norm);
                                $key = implode(',', array_unique($norm));
                                if ($key === '' || $key !== $setAtualKey) { continue; }
                                // Mesmo conjunto de datas: so excluir se for o oficio CANONICO
                                // (o que o fluxo de sobrescrever encontra/atualiza).
                                $hashCanon = sha1('POUPA TEMPO' . '|' . $ds);
                                $hashGrav  = isset($rd['hash_chave']) ? (string)$rd['hash_chave'] : '';
                                if ($hashGrav !== '' && $hashGrav === $hashCanon) {
                                    $excluirIdsJO[] = (int)$rd['id'];
                                }
                            }
                        }
                    }
                ...
                $notInJO = '';
                if (!empty($excluirIdsJO)) {
                    $excluirIdsJO = array_values(array_unique(array_map('intval', $excluirIdsJO)));
                    $notInJO = ' AND dl.id_despacho NOT IN (' . implode(',', $excluirIdsJO) . ') ';
                }

                $phJO = implode(',', array_fill(0, count($datasSqlJO), '?'));
                $sqlJO = "
                    SELECT LPAD(dl.posto,3,'0') AS posto,
                           dl.lote AS lote,
                           SUM(COALESCE(dl.quantidade,0)) AS quantidade
                    FROM ciDespachoLotes dl
                    INNER JOIN ciDespachoItens di
                            ON di.id_despacho = dl.id_despacho
                           AND LPAD(di.posto,3,'0') = LPAD(dl.posto,3,'0')
                    WHERE DATE(dl.data_carga) IN ($phJO)
                      $notInJO
                      AND (
                            (di.lacre_iipr     IS NOT NULL AND di.lacre_iipr     <> '' AND di.lacre_iipr     <> '0')
                         OR (di.lacre_correios IS NOT NULL AND di.lacre_correios <> '' AND di.lacre_correios <> '0')
                          )
                    GROUP BY LPAD(dl.posto,3,'0'), dl.lote
                    ORDER BY LPAD(dl.posto,3,'0'), dl.lote
                ";
                $stmtJO = $pdo_controle->prepare($sqlJO);
                $stmtJO->execute($datasSqlJO);
                while ($rowJO = $stmtJO->fetch(PDO::FETCH_ASSOC)) {
                    $pjo = (string)$rowJO['posto'];
                    $lotes_ja_em_oficio[] = array(
                        'posto'      => $pjo,
                        'lote'       => (string)$rowJO['lote'],
                        'quantidade' => (int)$rowJO['quantidade']
                    );
                    $postos_ja_em_oficio[$pjo] = true;
                }
            }
        }
    } catch (Exception $e) {
        // Banco fora (LAN) ou schema diferente: aviso nunca deve travar a pagina
        $lotes_ja_em_oficio  = array();
        $postos_ja_em_oficio = array();
    }
}

// v2.5.1: Lista INFORMATIVA (somente leitura) de TODOS os lotes/postos do periodo que JA
// constam em QUALQUER oficio salvo COM lacres — SEM a exclusao do oficio canonico. Diferente
// de $lotes_ja_em_oficio (que so aponta DUPLICIDADE real, escondendo o proprio oficio sendo
// regerado e por isso costuma mostrar so 1 posto), esta lista mostra TUDO que ja foi salvo no
// periodo, para o usuario ter ciencia de todos os postos/lotes. Considera lacre tanto em
// ciDespachoItens (lacre_iipr/lacre_correios) quanto em ciDespachoLotes (etiquetaiipr/
// etiquetacorreios). EXISTS evita multiplicar linhas no JOIN.
$lotes_periodo_info = array();
if (!empty($datas_filtro)) {
    try {
        $datasSqlInfo = array();
        foreach ($datas_filtro as $d) {
            $obj = DateTime::createFromFormat('d-m-Y', $d);
            if ($obj) { $datasSqlInfo[] = $obj->format('Y-m-d'); }
        }
        if (!empty($datasSqlInfo)) {
            $temLotesTabI = false;
            try { $rowsLTI = $pdo_controle->query("SHOW TABLES LIKE 'ciDespachoLotes'")->fetchAll(); $temLotesTabI = !empty($rowsLTI); } catch (Exception $e) { $temLotesTabI = false; }
            if ($temLotesTabI) {
                $phInfo = implode(',', array_fill(0, count($datasSqlInfo), '?'));
                $sqlInfo = "
                    SELECT LPAD(dl.posto,3,'0') AS posto,
                           dl.lote AS lote,
                           SUM(COALESCE(dl.quantidade,0)) AS quantidade
                    FROM ciDespachoLotes dl
                    WHERE DATE(dl.data_carga) IN ($phInfo)
                      AND (
                            (dl.etiquetaiipr     IS NOT NULL AND dl.etiquetaiipr     <> '' AND dl.etiquetaiipr     <> '0')
                         OR (dl.etiquetacorreios IS NOT NULL AND dl.etiquetacorreios <> '' AND dl.etiquetacorreios <> '0')
                         OR EXISTS (
                                SELECT 1 FROM ciDespachoItens di
                                WHERE di.id_despacho = dl.id_despacho
                                  AND LPAD(di.posto,3,'0') = LPAD(dl.posto,3,'0')
                                  AND (
                                        (di.lacre_iipr     IS NOT NULL AND di.lacre_iipr     <> '' AND di.lacre_iipr     <> '0')
                                     OR (di.lacre_correios IS NOT NULL AND di.lacre_correios <> '' AND di.lacre_correios <> '0')
                    ...
                    ORDER BY LPAD(dl.posto,3,'0'), dl.lote
                ";
                $stmtInfo = $pdo_controle->prepare($sqlInfo);
                $stmtInfo->execute($datasSqlInfo);
                while ($rowInfo = $stmtInfo->fetch(PDO::FETCH_ASSOC)) {
                    $lotes_periodo_info[] = array(
                        'posto'      => (string)$rowInfo['posto'],
                        'lote'       => (string)$rowInfo['lote'],
                        'quantidade' => (int)$rowInfo['quantidade']
                    );
                }
            }
        }
...
// v2.5.2: Maior lacre usado (IIPR e Correios) do ULTIMO oficio Correios gerado, para exibir
// na tela. A v2.5.1 fazia MAX GLOBAL e trazia numeros fora do padrao (ex.: dados antigos ou
// numeracao diferente) -> mostrou 25.022.556 / 2.504.224 em vez de 53708 / 53709. O correto e
// escopar ao ULTIMO despacho grupo='correios' (o oficio mais recente), de onde sai o "proximo
// lacre a usar". Le as DUAS fontes DESSE despacho: ciDespachoItens.lacre_iipr/lacre_correios
// (onde ficam as colunas Lacre IIPR / Lacre Correios do comprovante) E, como fallback,
// ciDespachoLotes.etiquetaiipr/etiquetacorreios — pegando o MAIOR. Faixa com hifen
// ("12430-12431" -> usa o lado direito) via SUBSTRING_INDEX. try/catch: banco fora -> 0.
$ultimo_lacre_iipr = 0;
...
try {
    ...
    // Silenciar erro (banco fora)
}

// Helper: roda uma query parametrizada de MAX e devolve int (0 em erro/sem dados).
$lacreMaxScoped = function ($pdo, $sql, $params) {
    try {
        $st = $pdo->prepare($sql);
        $st->execute($params);
        $v = $st->fetchColumn();
...
if ($ultimo_despacho_correios > 0) {
    // Para faixa com hifen, pega o numero APOS o ultimo hifen (lado direito = maior da faixa).
    $exprIiprItens = "CAST(SUBSTRING_INDEX(lacre_iipr, '-', -1) AS UNSIGNED)";
    $exprCorrItens = "CAST(SUBSTRING_INDEX(lacre_correios, '-', -1) AS UNSIGNED)";
    $cands_iipr = array(
        $lacreMaxScoped($pdo_controle, "SELECT MAX($exprIiprItens) FROM ciDespachoItens WHERE id_despacho = ? AND lacre_iipr IS NOT NULL AND lacre_iipr <> '' AND lacre_iipr <> '0'", array($ultimo_despacho_correios)),
        $lacreMaxScoped($pdo_controle, "SELECT MAX(CAST(etiquetaiipr AS UNSIGNED)) FROM ciDespachoLotes WHERE id_despacho = ? AND etiquetaiipr IS NOT NULL AND etiquetaiipr <> '' AND etiquetaiipr <> '0'", array($ultimo_despacho_correios))
    );
    $cands_corr = array(
        $lacreMaxScoped($pdo_controle, "SELECT MAX($exprCorrItens) FROM ciDespachoItens WHERE id_despacho = ? AND lacre_correios IS NOT NULL AND lacre_correios <> '' AND lacre_correios <> '0'", array($ultimo_despacho_correios)),
        $lacreMaxScoped($pdo_controle, "SELECT MAX(CAST(etiquetacorreios AS UNSIGNED)) FROM ciDespachoLotes WHERE id_despacho = ? AND etiquetacorreios IS NOT NULL AND etiquetacorreios <> '' AND etiquetacorreios <> '0'", array($ultimo_despacho_correios))
    );
    $ultimo_lacre_iipr = max($cands_iipr);
    $ultimo_lacre_correios = max($cands_corr);
}

// v2.7.0 (B): nunca deixar 0/branco. Se o ULTIMO oficio Correios nao tinha aquele lacre
// (ficou 0), buscar o MAIOR lacre ja usado em QUALQUER oficio grupo='correios' (JOIN ate
// ciDespachos -> NUNCA e MAX global sem filtro, logo nao reaparece o bug historico dos
// ~25 milhoes). Le ciDespachoItens E ciDespachoLotes; mesma normalizacao de faixa (hifen).
if ($ultimo_lacre_iipr <= 0) {
    $cands_iipr_g = array(
        $lacreMaxScoped($pdo_controle, "SELECT MAX(CAST(SUBSTRING_INDEX(i.lacre_iipr, '-', -1) AS UNSIGNED)) FROM ciDespachoItens i INNER JOIN ciDespachos d ON d.id = i.id_despacho WHERE LOWER(d.grupo) = 'correios' AND i.lacre_iipr IS NOT NULL AND i.lacre_iipr <> '' AND i.lacre_iipr <> '0'", array()),
        $lacreMaxScoped($pdo_controle, "SELECT MAX(CAST(l.etiquetaiipr AS UNSIGNED)) FROM ciDespachoLotes l INNER JOIN ciDespachos d ON d.id = l.id_despacho WHERE LOWER(d.grupo) = 'correios' AND l.etiquetaiipr IS NOT NULL AND l.etiquetaiipr <> '' AND l.etiquetaiipr <> '0'", array())
    );
    $ultimo_lacre_iipr = max($cands_iipr_g);
}
if ($ultimo_lacre_correios <= 0) {
    $cands_corr_g = array(
        $lacreMaxScoped($pdo_controle, "SELECT MAX(CAST(SUBSTRING_INDEX(i.lacre_correios, '-', -1) AS UNSIGNED)) FROM ciDespachoItens i INNER JOIN ciDespachos d ON d.id = i.id_despacho WHERE LOWER(d.grupo) = 'correios' AND i.lacre_correios IS NOT NULL AND i.lacre_correios <> '' AND i.lacre_correios <> '0'", array()),
        $lacreMaxScoped($pdo_controle, "SELECT MAX(CAST(l.etiquetacorreios AS UNSIGNED)) FROM ciDespachoLotes l INNER JOIN ciDespachos d ON d.id = l.id_despacho WHERE LOWER(d.grupo) = 'correios' AND l.etiquetacorreios IS NOT NULL AND l.etiquetacorreios <> '' AND l.etiquetacorreios <> '0'", array())
    );
    $ultimo_lacre_correios = max($cands_corr_g);
}
...
if (!empty($_SESSION['postos_manuais'])) {
    foreach ($_SESSION['postos_manuais'] as $codigo => $posto) {
        ...
        if ($temReferencia) {
            ...
            $postos_manuais_inserir[$grupoRef][] = array(
                'posto_codigo' => $codigo,
                'posto_real' => isset($posto['posto_real']) ? $posto['posto_real'] : '',
...
// v2.9.8: aplica o filtro de grupo (Correios x Poupa Tempo). 'correios' = CAPITAL +
// CENTRAL IIPR + REGIONAIS (tudo que NAO e POUPA TEMPO); 'poupatempo' = so POUPA TEMPO.
$dados = array('POUPA TEMPO' => array(), 'CAPITAL' => array(), 'CENTRAL IIPR' => array(), 'REGIONAIS' => array());
foreach ($postos_visiveis as $posto) {
    if ($filtro_grupo === 'poupatempo' && $posto['tipo'] !== 'POUPA TEMPO') { continue; }
    if ($filtro_grupo === 'correios' && $posto['tipo'] === 'POUPA TEMPO') { continue; }
    ...
    foreach ($postos_manuais_inserir as $grupoRef => $listaInsercoes) {
        ...
        foreach ($listaInsercoes as $novoPosto) {
            ...
            $entry = array(
                ...
                'posto_real' => isset($novoPosto['posto_real']) ? $novoPosto['posto_real'] : '',
...
// v2.9.5: RESTAURAR a GRADE a partir do SNAPSHOT do oficio salvo quando o operador
// FILTRA por uma data que ja tem oficio Correios gravado. Reconstroi o oficio COMPLETO:
// (a) linhas adicionadas MANUALMENTE (ex.: postos 507/808), com numero real + nome — que
// somem da sessao no dia seguinte; (b) os lacres EXATOS como digitados, inclusive faixa
// com hifen (ex.: "53755-53756", que hoje volta truncado "53756"). Assim da p/ continuar
// o oficio em outro dia. SO roda em load por GET com filtro de datas (nunca em POST de
// edicao/save) — evita reintroduzir o bug do auto-save que sobrescrevia inputs vivos.
$snapshotRestaurado = false;
$ehGetSnap = (isset($_SERVER['REQUEST_METHOD']) && strtoupper($_SERVER['REQUEST_METHOD']) === 'GET');
// Nao restaurar durante um recalculo por lacre (GET ou POST): o recalculo reescreve os
// lacres do grupo logo abaixo (l.~5177+) e o snapshot conflitaria com ele.
$ehRecalculoSnap = ((isset($_GET['recalculo_por_lacre']) && $_GET['recalculo_por_lacre'] === '1') || (isset($_POST['recalculo_por_lacre']) && $_POST['recalculo_por_lacre'] === '1'));
// v2.9.5: diagnostico opcional (?diag_snapshot=1) p/ inspecionar a restauracao sem acesso ao banco.
$diagSnap = array('ran' => false, 'reason' => '', 'filtro' => '', 'match_id' => 0, 'match_tipo' => '', 'match_datas' => '', 'snap_len' => 0, 'snap_linhas' => 0, 'snap_manuais' => 0, 'criadas' => 0, 'candidatos' => array());
if ($acabouDeLimpar) { $diagSnap['reason'] = 'acabou de limpar sessao'; }
elseif (!$ehGetSnap) { $diagSnap['reason'] = 'requisicao nao e GET'; }
elseif ($ehRecalculoSnap) { $diagSnap['reason'] = 'recalculo_por_lacre ativo'; }
elseif (empty($datas_filtro)) { $diagSnap['reason'] = 'sem filtro de datas'; }
if (!$acabouDeLimpar && $ehGetSnap && !$ehRecalculoSnap && !empty($datas_filtro)) {
    try {
        // 1) Conjunto de datas do filtro normalizado p/ YYYY-MM-DD ordenado
        $setFiltroSnap = array();
        foreach ($datas_filtro as $dFil) {
            ...
            if (preg_match('/^\d{4}-\d{2}-\d{2}$/', $dFil)) {
                ...
                if ($objFil) { $setFiltroSnap[] = $objFil->format('Y-m-d'); }
            }
        }
        sort($setFiltroSnap);
        $setFiltroArr = array_values(array_unique($setFiltroSnap));
        $setFiltroKeySnap = implode(',', $setFiltroArr);
        ...
        if ($setFiltroKeySnap !== '') {
            ...
            if ($stDespSnap) {
                while ($rdSnap = $stDespSnap->fetch(PDO::FETCH_ASSOC)) {
                    $dsSnap = isset($rdSnap['datas_str']) ? (string)$rdSnap['datas_str'] : '';
                    $partsSnap = preg_split('/\s*,\s*/', trim($dsSnap));
                    $normSnap = array();
                    foreach ($partsSnap as $pSnap) {
                        $pSnap = trim($pSnap);
                        if ($pSnap === '') { continue; }
                        if (preg_match('/^\d{4}-\d{2}-\d{2}$/', $pSnap)) {
                            $normSnap[] = $pSnap;
                        } elseif (preg_match('#^(\d{2})[/-](\d{2})[/-](\d{4})$#', $pSnap, $mmSnap)) {
                            $normSnap[] = $mmSnap[3] . '-' . $mmSnap[2] . '-' . $mmSnap[1];
                        }
                    }
                    sort($normSnap);
            ...
            // 3) Aplicar o snapshot a grade ($dados)
            if ($snapJsonEncontrado !== '') {
                $snapRows = json_decode($snapJsonEncontrado, true);
                if (is_array($snapRows) && !empty($snapRows)) {
                    $gruposValidosSnap = array('CAPITAL' => true, 'CENTRAL IIPR' => true, 'REGIONAIS' => true);
                    $diagSnap['snap_linhas'] = count($snapRows);
                    for ($dm = 0; $dm < count($snapRows); $dm++) { if (is_array($snapRows[$dm]) && !empty($snapRows[$dm]['manual_inserido'])) { $diagSnap['snap_manuais']++; } }

                    // 3a) Index dos snaps por chave de posto (crua + pad3). O 'posto' do
                    //     snapshot e o MESMO data-posto-codigo da grade -> casa direto
                    //     (inclusive regionais), sem a heuristica multi-chave antiga.
                    $snapPorChave = array();
                    for ($si = 0; $si < count($snapRows); $si++) {
                        if (!is_array($snapRows[$si]) || !isset($snapRows[$si]['posto'])) { continue; }
                        $pRaw = trim((string)$snapRows[$si]['posto']);
                        if ($pRaw === '') { continue; }
                        $snapRows[$si]['_usado'] = false;
                        if (!isset($snapPorChave[$pRaw])) { $snapPorChave[$pRaw] = $si; }
                        if (!preg_match('/^M/i', $pRaw)) {
                            $digSr = preg_replace('/\D/', '', $pRaw);
                            if ($digSr !== '') {
                                $padSr = str_pad($digSr, 3, '0', STR_PAD_LEFT);
                                if (!isset($snapPorChave[$padSr])) { $snapPorChave[$padSr] = $si; }
                            }
                        }
                    }

                    // 3b) PATCH nas linhas existentes: lacres EXATOS (com hifen) + etiqueta
                    foreach ($dados as $grpNome => &$itensGrp) {
                        if ($grpNome === 'POUPA TEMPO') { continue; } // fluxo PT nao usa snapshot Correios
                        foreach ($itensGrp as &$linhaGrp) {
                            $codLinha = isset($linhaGrp['posto_codigo']) ? trim((string)$linhaGrp['posto_codigo']) : '';
                            if ($codLinha === '') { continue; }
                            $chavesLinha = array($codLinha);
                            if (!preg_match('/^M/i', $codLinha)) {
                                $digLinha = preg_replace('/\D/', '', $codLinha);
                                if ($digLinha !== '') { $chavesLinha[] = str_pad($digLinha, 3, '0', STR_PAD_LEFT); }
                            }
                            $idxSnap = null;
                            for ($kc = 0; $kc < count($chavesLinha); $kc++) {
                                if (isset($snapPorChave[$chavesLinha[$kc]])) { $idxSnap = $snapPorChave[$chavesLinha[$kc]]; break; }
                            }
                            if ($idxSnap === null) { continue; }
                            $srMatch = $snapRows[$idxSnap];
                            $snapRows[$idxSnap]['_usado'] = true;
                            $linhaGrp['lacre_iipr'] = isset($srMatch['lacre_iipr']) ? (string)$srMatch['lacre_iipr'] : '';
                            $linhaGrp['lacre_correios'] = isset($srMatch['lacre_correios']) ? (string)$srMatch['lacre_correios'] : '';
                            $etiSnap = isset($srMatch['etiqueta_correios']) ? trim((string)$srMatch['etiqueta_correios']) : '';
                            if ($etiSnap !== '') { $_SESSION['etiquetas'][$codLinha] = $etiSnap; }
                        }
                        unset($linhaGrp);
                    }
                    unset($itensGrp);

                    // 3c) CRIAR linhas do snapshot ausentes na grade (manuais/fora do filtro).
                    //     Respeita exclusoes manuais da sessao corrente (linhas_removidas).
                    for ($si = 0; $si < count($snapRows); $si++) {
                        if (!is_array($snapRows[$si]) || empty($snapRows[$si]['posto'])) { continue; }
                        if (!empty($snapRows[$si]['_usado'])) { continue; }
                        $grpSr = isset($snapRows[$si]['grupo']) ? trim((string)$snapRows[$si]['grupo']) : '';
                        if ($grpSr === 'POUPA TEMPO') { continue; }
                        $grpDestino = isset($gruposValidosSnap[$grpSr]) ? $grpSr : 'REGIONAIS';
                        $pRaw = trim((string)$snapRows[$si]['posto']);
                        // v2.9.5: o snapshot vem do POST -> sanitizar o codigo do posto antes
                        // de criar a linha. Ele e renderizado CRU em data-posto-codigo e em
                        // onclick handlers. Aceita so numeros ou codigo manual "M...".
                        if (!preg_match('/^[0-9]+$/', $pRaw) && !preg_match('/^M[A-Za-z0-9_\-]+$/', $pRaw)) { continue; }
                        if (isset($_SESSION['linhas_removidas'][$pRaw]) && $_SESSION['linhas_removidas'][$pRaw] === true) { continue; }
                        $nomeSr = (isset($snapRows[$si]['posto_nome']) && trim((string)$snapRows[$si]['posto_nome']) !== '') ? trim((string)$snapRows[$si]['posto_nome']) : $pRaw;
                        // o nome aparece CRU numa celula (sink existente) -> remover < e >
                        $nomeSr = str_replace(array('<', '>'), '', $nomeSr);
                        $novaLinhaSnap = array(
                            'posto_codigo' => $pRaw,
                            'posto_real' => isset($snapRows[$si]['posto_real']) ? (string)$snapRows[$si]['posto_real'] : '',
                            'posto_nome' => $nomeSr,
                            'tipo' => $grpDestino,
                            'quantidade' => 1,
                            'regional' => isset($snapRows[$si]['regional']) ? (string)$snapRows[$si]['regional'] : '0',
                            'manual_inserido' => 1,
                            'lacre_iipr' => isset($snapRows[$si]['lacre_iipr']) ? (string)$snapRows[$si]['lacre_iipr'] : '',
                            'lacre_correios' => isset($snapRows[$si]['lacre_correios']) ? (string)$snapRows[$si]['lacre_correios'] : ''
                        );
                        if (!isset($dados[$grpDestino])) { $dados[$grpDestino] = array(); }
                        $dados[$grpDestino][] = $novaLinhaSnap;
                        $diagSnap['criadas']++;
                        $etiSr = isset($snapRows[$si]['etiqueta_correios']) ? trim((string)$snapRows[$si]['etiqueta_correios']) : '';
                        if ($etiSr !== '') { $_SESSION['etiquetas'][$pRaw] = $etiSr; }
                    }

                    $snapshotRestaurado = true;
                    $diagSnap['ran'] = true;
                }
            }
        }
    } catch (Exception $eSnapRest) {
        $snapshotRestaurado = false; // fail-open: cai no prefill numerico antigo abaixo
    }
}
...
try {
    ...
    if ($ultimoOficioId > 0) {
        ...
        // v2.6.0 (I2/I3): casar por MULTIPLAS chaves candidatas (posto_codigo, posto_real
        // e regional), cada uma na forma crua e com pad3. Necessario porque em REGIONAIS o
        // ciDespachoLotes.posto pode ter sido salvo como o NUMERO regional (ex.: 779/800/950)
        // e nao como o posto_codigo interno (ex.: 151/969) -> o casamento so-por-posto_codigo
        // falhava (lacres e displays das regionais nao pre-preenchiam). Prioridade
        // posto_codigo > posto_real > regional evita colisao; fail-safe: a 1a chave existente
        // em $lacresOficio vence. PHP 5.3.3 (sem sintaxe moderna).
        foreach ($dados as $grupo => &$itens) {
            foreach ($itens as &$posto) {
                $codigo = $posto['posto_codigo'];
                $camposChave = array();
                $camposChave[] = isset($posto['posto_codigo']) ? $posto['posto_codigo'] : '';
                $camposChave[] = isset($posto['posto_real']) ? $posto['posto_real'] : '';
                $camposChave[] = isset($posto['regional']) ? $posto['regional'] : '';
                $chavesTentar = array();
                for ($ic = 0; $ic < count($camposChave); $ic++) {
                    $valChave = trim((string)$camposChave[$ic]);
                    if ($valChave === '') { continue; }
                    $chavesTentar[] = $valChave; // forma crua (cobre M... sinteticos)
                    if (!preg_match('/^M/i', $valChave)) {
                        $dig = preg_replace('/\D/', '', $valChave);
                        if ($dig !== '' && (int)$dig > 0) {
                            $chavesTentar[] = str_pad($dig, 3, '0', STR_PAD_LEFT);
                        }
                    }
                }
                $encontrado = null;
                for ($jc = 0; $jc < count($chavesTentar); $jc++) {
                    $ct = $chavesTentar[$jc];
                    if ($ct !== '' && isset($lacresOficio[$ct])) {
                        $encontrado = $lacresOficio[$ct];
                        break;
                    }
                }
                if ($encontrado !== null) {
                    $posto['lacre_iipr'] = $encontrado['lacre_iipr'];
                    $posto['lacre_correios'] = $encontrado['lacre_correios'];
                    if (!empty($encontrado['etiqueta_correios'])) {
                        $_SESSION['etiquetas'][$codigo] = $encontrado['etiqueta_correios'];
                    }
                }
            }
            unset($posto);
...
// Resolve a CHAVE de estoque de display usada por um posto da grade.
// Regra unica compartilhada entre o RENDER e o diagnostico (?diag_displays) para
// nunca divergirem: Central/regional 999 -> 'central'; no grupo REGIONAIS o estoque
// e chaveado pelo NUMERO do posto (campo 'regional', ex.: 700/950) e NAO pelo
// posto_codigo interno (ex.: 151/969); demais grupos usam o posto_codigo.
// Guard por isset = fail-safe (so troca quando a chave existe no estoque).
if (!function_exists('lacresResolverChaveStatus')) {
    function lacresResolverChaveStatus($grupo, $posto_codigo, $regional, $status_postos) {
        $regional_num = (int)preg_replace('/\D/', '', (string)$regional);
        $usa_central = ($grupo === 'CENTRAL IIPR') || ($regional_num === 999);
        $chave = ($usa_central && isset($status_postos['central'])) ? 'central' : $posto_codigo;
        if (!$usa_central && $grupo === 'REGIONAIS' && $regional_num > 0) {
            $reg_pad = str_pad((string)$regional_num, 3, '0', STR_PAD_LEFT);
            if ($reg_pad !== '000' && isset($status_postos[$reg_pad])) {
                $chave = $reg_pad;
            }
        }
        return $chave;
...
try {
    ...
    if (lacresTabelaExiste($pdo_controle, 'ciMalotes')) {
        ...
        // Inventario fisico (ciInventarioDisplays, carregado em conferencia_inventario.php):
        // um display presente no inventario voltou fisicamente, mesmo que o ciMalotes
        // ainda nao tenha o retorno (tipo=2). Logo ele NAO conta como "em transito" e
        // nao deve gerar falso "Aguardando display".
        $temInventario = lacresTabelaExiste($pdo_controle, 'ciInventarioDisplays');

        // 1) Total de displays cadastrados por posto (universo conhecido)
        if ($temCadastro) {
            try {
                // So conta como "cadastrado do posto N" quando o posto e PURAMENTE
                // numerico (REGEXP '^[0-9]+$'). Rotulos como '250*' pertencem a outro
                // lugar (movem-se por ciMalotes, ex.: posto='202') e NAO devem inflar o
                // total do posto 250 — senao 'cadastrados' viraria 12 quando o posto
                // real tem 6. Mesma regra ja usada nas consultas de transito/presente.
                $stReg = $pdo_controle->query(
                    "SELECT LPAD(CAST(posto AS UNSIGNED),3,'0') AS posto_pad, COUNT(*) AS total
                     FROM cadastroMalotes
                     WHERE posto IS NOT NULL AND posto <> '' AND TRIM(posto) REGEXP '^[0-9]+$'
                     GROUP BY posto_pad"
                );
        ...
        try {
            ...
            $joinInv  = $temInventario ? "LEFT JOIN ciInventarioDisplays inv ON inv.leitura = mt.leitura" : "";
            $whereInv = $temInventario ? "WHERE inv.leitura IS NULL" : "";
        ...
        // 2c) Displays FISICAMENTE PRESENTES por posto (estoque LOCAL real).
        //     Um display esta presente (local) quando:
        //       (a) consta no inventario fisico (ciInventarioDisplays) — prova fisica; ou
        //       (b) seu ULTIMO movimento em ciMalotes e tipo=2 (retornou) e nao esta no inventario.
        //     Isso conserta o falso "0 local": um posto pode nao estar no cadastroMalotes
        //     (entao "cadastrados - transito" da 0 ou negativo) mas TER displays no inventario.
        //     Posto resolvido pelo cadastro (numerico) senao pelo posto do inventario/ciMalotes.
        $estoque_presente = array();
        // (a) inventario fisico
        if ($temInventario) {
            try {
                $postoInvExpr = $temCadastro
                    ? "LPAD(CAST(CASE WHEN c.posto REGEXP '^[0-9]+$' THEN c.posto ELSE inv.posto END AS UNSIGNED),3,'0')"
                    : "LPAD(CAST(inv.posto AS UNSIGNED),3,'0')";
                $joinCadInv = $temCadastro ? "LEFT JOIN cadastroMalotes c ON c.leitura = inv.leitura" : "";
                $stInv = $pdo_controle->query(
                    "SELECT " . $postoInvExpr . " AS posto_pad, COUNT(DISTINCT inv.leitura) AS qt
                       FROM ciInventarioDisplays inv
                       " . $joinCadInv . "
                      GROUP BY posto_pad"
                );
                while ($r = $stInv->fetch(PDO::FETCH_ASSOC)) {
                    $cod = $r['posto_pad'];
                    if ($cod === null || $cod === '') continue;
                    if (!isset($estoque_presente[$cod])) $estoque_presente[$cod] = 0;
                    $estoque_presente[$cod] += (int)$r['qt'];
                }
            } catch (Exception $e) { /* tolera */ }
        }
        // (b) retornados (ultimo movimento = tipo 2) que nao estao no inventario
        try {
            $postoRetExpr = $temCadastro
                ? "LPAD(CAST(CASE WHEN c.posto REGEXP '^[0-9]+$' THEN c.posto ELSE mt.posto END AS UNSIGNED),3,'0')"
                : "LPAD(CAST(mt.posto AS UNSIGNED),3,'0')";
            $joinCadRet = $temCadastro ? "LEFT JOIN cadastroMalotes c ON c.leitura = mt.leitura" : "";
            $joinInvRet = $temInventario ? "LEFT JOIN ciInventarioDisplays inv ON inv.leitura = mt.leitura" : "";
            $whereInvRet = $temInventario ? "WHERE inv.leitura IS NULL" : "";
            $stRet = $pdo_controle->query(
                "SELECT " . $postoRetExpr . " AS posto_pad, COUNT(DISTINCT mt.leitura) AS qt
                   FROM (
            ...
            while ($r = $stRet->fetch(PDO::FETCH_ASSOC)) {
                $cod = $r['posto_pad'];
                if ($cod === null || $cod === '') continue;
                if (!isset($estoque_presente[$cod])) $estoque_presente[$cod] = 0;
                $estoque_presente[$cod] += (int)$r['qt'];
        ...
        foreach ($estoque_presente as $cod => $v)  { $postos_conhecidos[$cod] = true; }
        foreach ($postos_conhecidos as $cod => $v) {
            ...
            $presente = isset($estoque_presente[$cod]) ? $estoque_presente[$cod] : 0;
            // Disponivel local = MAIOR entre o calculo por cadastro (cadastrados - em transito)
            // e a evidencia fisica (inventario + retornados). Assim o inventario/retorno
            // conserta o falso "0 local" quando o posto nao consta no cadastroMalotes.
            $disp_reg = $total - $transito;
            $disponivel = ($disp_reg > $presente) ? $disp_reg : $presente;
            lacresGarantirStatus($status_postos, $cod);
            $status_postos[$cod]['estoque_total'] = $total;
            $status_postos[$cod]['estoque_transito'] = $transito;
            $status_postos[$cod]['estoque_presente'] = $presente;
        ...
        // 2b) Estoque da CENTRAL (displays cadastrados com posto textual, ex.: 'central').
        //     Postos que pertencem a Central (regional 999) NAO tem display proprio:
        //     recebem o display da Central. Logo a disponibilidade deles deve refletir
        //     o estoque da Central, e nao o bucket do proprio posto (que so acumula
        //     displays da Central "em transito" para aquele posto -> falso "Aguardando").
        if ($temCadastro) {
            try {
                // Apenas displays da CENTRAL (posto textual 'central'). NAO usar
                // "nao numerico", pois existem postos reais nao-numericos (ex.: 250*, 450*).
                $stCentTot = $pdo_controle->query(
                    "SELECT COUNT(*) AS total FROM cadastroMalotes
                      WHERE posto IS NOT NULL AND LOWER(TRIM(posto)) LIKE '%central%'"
                );
                $rowCT = $stCentTot ? $stCentTot->fetch(PDO::FETCH_ASSOC) : null;
                $centralTotal = $rowCT ? (int)$rowCT['total'] : 0;

                $stCentTr = $pdo_controle->query(
                    "SELECT COUNT(*) AS qt, MAX(DATE(mt.data)) AS data_mov
                       FROM (
                           SELECT m1.leitura, m1.data
                             FROM ciMalotes m1
                             INNER JOIN (SELECT leitura, MAX(id) AS maxid FROM ciMalotes GROUP BY leitura) lt
                                     ON m1.id = lt.maxid
                            WHERE m1.tipo = 1
                       ) mt
                       INNER JOIN cadastroMalotes c ON c.leitura = mt.leitura
                       " . ($temInventario ? "LEFT JOIN ciInventarioDisplays inv ON inv.leitura = mt.leitura" : "") . "
                      WHERE c.posto IS NOT NULL AND LOWER(TRIM(c.posto)) LIKE '%central%'"
                       . ($temInventario ? " AND inv.leitura IS NULL" : "")
                );
                $rowCTr = $stCentTr ? $stCentTr->fetch(PDO::FETCH_ASSOC) : null;
                $centralTransito = $rowCTr ? (int)$rowCTr['qt'] : 0;
                $centralData = ($rowCTr && isset($rowCTr['data_mov'])) ? $rowCTr['data_mov'] : '';

                // Presentes (local) da Central: inventario fisico + retornados (ultimo mov tipo=2).
                $centralPresente = 0;
                if ($temInventario) {
                    try {
                        $stCP = $pdo_controle->query(
                            "SELECT COUNT(DISTINCT inv.leitura) AS qt
                               FROM ciInventarioDisplays inv
                               INNER JOIN cadastroMalotes c ON c.leitura = inv.leitura
                              WHERE c.posto IS NOT NULL AND LOWER(TRIM(c.posto)) LIKE '%central%'"
                        );
                        $rcp = $stCP ? $stCP->fetch(PDO::FETCH_ASSOC) : null;
                        if ($rcp) $centralPresente += (int)$rcp['qt'];
                    } catch (Exception $e) { /* tolera */ }
                }
                try {
                    $stCRet = $pdo_controle->query(
                        "SELECT COUNT(DISTINCT mt.leitura) AS qt
                           FROM (
                               SELECT m1.leitura
                                 FROM ciMalotes m1
                                 INNER JOIN (SELECT leitura, MAX(id) AS maxid FROM ciMalotes GROUP BY leitura) lt
                    ...
                    $rcr = $stCRet ? $stCRet->fetch(PDO::FETCH_ASSOC) : null;
                    if ($rcr) $centralPresente += (int)$rcr['qt'];
                } catch (Exception $e) { /* tolera */ }

                $centralReg = $centralTotal - $centralTransito;
                $centralDisp = ($centralReg > $centralPresente) ? $centralReg : $centralPresente;
                lacresGarantirStatus($status_postos, 'central');
                $status_postos['central']['estoque_total'] = $centralTotal;
                $status_postos['central']['estoque_transito'] = $centralTransito;
                $status_postos['central']['estoque_presente'] = $centralPresente;
                $status_postos['central']['estoque_disponivel'] = $centralDisp;
                if ($centralDisp <= 0 && ($centralTotal > 0 || $centralTransito > 0 || $centralPresente > 0)) {
                    $status_postos['central']['display'] = 'fora';
                    $status_postos['central']['display_data'] = $centralData;
                }
            } catch (Exception $e) { /* tolera */ }
        }
    }
...
// === DIAGNOSTICO TEMPORARIO de estoque de displays (REMOVER apos uso) ===
// Abrir no navegador (ambiente do usuario, com banco acessivel):
//   lacres_novo.php?diag_displays=250
// Mostra os numeros CRUS que a grade usou + cadastroMalotes/ciMalotes/inventario
// das etiquetas daquele posto, para localizar por que "local" aparece zerado.
if (isset($_GET['diag_displays'])) {
    header('Content-Type: text/plain; charset=utf-8');
    $alvo = preg_replace('/\D/', '', (string)$_GET['diag_displays']);
    $alvoKey = ($alvo === '') ? '' : str_pad($alvo, 3, '0', STR_PAD_LEFT);
    echo "=== DIAGNOSTICO DISPLAYS posto '" . $alvo . "' (chave da grade: '" . $alvoKey . "') ===\n\n";

    echo "[0] Linhas da GRADE (\$dados) cujo posto_codigo/regional batem com '" . $alvo . "':\n";
    $achouLinha = false;
    if (isset($dados) && is_array($dados)) {
        foreach ($dados as $grp => $itens) {
            if (!is_array($itens)) continue;
            foreach ($itens as $it) {
                if (!is_array($it)) continue;
                $pc  = isset($it['posto_codigo']) ? (string)$it['posto_codigo'] : '';
                $reg = isset($it['regional']) ? (string)$it['regional'] : '';
                $pcNum  = preg_replace('/\D/', '', $pc);
                $regNum = preg_replace('/\D/', '', $reg);
                if ($pcNum !== $alvo && $regNum !== $alvo) continue;
                $achouLinha = true;
                // Usa a MESMA regra do render (helper compartilhado) para nao enganar o diag.
                $chave = lacresResolverChaveStatus($grp, $pc, $reg, $status_postos);
                echo "  grupo='" . $grp . "' posto_codigo='" . $pc . "' regional='" . $reg
                   . "' nome='" . (isset($it['posto_nome']) ? $it['posto_nome'] : '') . "'\n";
                echo "      -> chave_status que a grade usa = '" . $chave . "' (alvoKey='" . $alvoKey . "')\n";
                if (isset($status_postos[$chave])) {
                    $sx = $status_postos[$chave];
                    echo "      -> status['" . $chave . "']: total=" . (isset($sx['estoque_total']) ? $sx['estoque_total'] : 'n/d')
                       . " transito=" . (isset($sx['estoque_transito']) ? $sx['estoque_transito'] : 'n/d')
                       . " presente=" . (isset($sx['estoque_presente']) ? $sx['estoque_presente'] : 'n/d')
                       . " disponivel=" . (isset($sx['estoque_disponivel']) ? $sx['estoque_disponivel'] : 'n/d') . "\n";
                } else {
                    echo "      -> SEM status_postos['" . $chave . "'] => chave da grade NAO existe no estoque (MISMATCH!)\n";
                }
            }
        }
    }
    if (!$achouLinha) { echo "  (nenhuma linha da grade bateu posto_codigo/regional = '" . $alvo . "')\n"; }
    echo "\n";

    echo "[1] Valores que a GRADE usou (status_postos['" . $alvoKey . "']):\n";
    if (isset($status_postos[$alvoKey])) {
        $s = $status_postos[$alvoKey];
        echo "  total=" . (isset($s['estoque_total']) ? $s['estoque_total'] : 'n/d')
           . "  transito=" . (isset($s['estoque_transito']) ? $s['estoque_transito'] : 'n/d')
           . "  presente=" . (isset($s['estoque_presente']) ? $s['estoque_presente'] : 'n/d')
           . "  disponivel=" . (isset($s['estoque_disponivel']) ? $s['estoque_disponivel'] : 'n/d')
           . "  display=" . (isset($s['display']) ? ($s['display'] === '' ? '(vazio)' : $s['display']) : '(vazio)') . "\n\n";
    } else {
        echo "  (NENHUMA entrada em status_postos['" . $alvoKey . "'] — chave nao bate!)\n\n";
    }

    $leituras = array();
    try {
        $q = $pdo_controle->prepare("SELECT id, leitura, cep, sequencial, posto FROM cadastroMalotes WHERE CAST(posto AS UNSIGNED)=? OR TRIM(posto)=? ORDER BY sequencial");
        $q->execute(array((int)$alvo, $alvo));
        echo "[2] cadastroMalotes (posto " . $alvo . "):\n";
        $n = 0;
        while ($r = $q->fetch(PDO::FETCH_ASSOC)) {
            $n++; $leituras[] = $r['leitura'];
            echo "  #" . $r['id'] . " leitura=" . $r['leitura'] . " cep=" . $r['cep'] . " seq=" . $r['sequencial'] . " posto='" . $r['posto'] . "'\n";
        }
        echo "  >>> " . $n . " etiqueta(s) cadastrada(s)\n\n";
    } catch (Exception $e) { echo "[2] erro: " . $e->getMessage() . "\n\n"; }

    if (!empty($leituras)) {
        try {
            $ph = implode(',', array_fill(0, count($leituras), '?'));
            $q = $pdo_controle->prepare("SELECT id, leitura, DATE(data) AS d, tipo, posto FROM ciMalotes WHERE leitura IN (" . $ph . ") ORDER BY leitura, id");
            $q->execute($leituras);
            echo "[3] ciMalotes dessas etiquetas (o MAIOR id de cada leitura = ultimo movimento):\n";
            $n = 0;
            while ($r = $q->fetch(PDO::FETCH_ASSOC)) {
                $n++;
                $tt = ($r['tipo'] === '1' || $r['tipo'] === 1) ? 'SAIDA(1)' : (($r['tipo'] === '2' || $r['tipo'] === 2) ? 'RETORNO(2)' : ('tipo=' . $r['tipo']));
                echo "  id=" . $r['id'] . " leitura=" . $r['leitura'] . " data=" . $r['d'] . " " . $tt . " posto='" . $r['posto'] . "'\n";
            }
            echo "  >>> " . $n . " movimento(s)\n\n";
        } catch (Exception $e) { echo "[3] erro: " . $e->getMessage() . "\n\n"; }

        if (lacresTabelaExiste($pdo_controle, 'ciInventarioDisplays')) {
            try {
                $ph = implode(',', array_fill(0, count($leituras), '?'));
                $q = $pdo_controle->prepare("SELECT * FROM ciInventarioDisplays WHERE leitura IN (" . $ph . ")");
                $q->execute($leituras);
                echo "[4] ciInventarioDisplays dessas etiquetas:\n";
                $n = 0;
                while ($r = $q->fetch(PDO::FETCH_ASSOC)) { $n++; echo "  " . json_encode($r) . "\n"; }
                echo "  >>> " . $n . " no inventario\n\n";
            } catch (Exception $e) { echo "[4] erro: " . $e->getMessage() . "\n\n"; }
        } else {
            echo "[4] ciInventarioDisplays nao existe\n\n";
        }
    } else {
        echo "[3/4] sem etiquetas cadastradas para esse posto — nada a casar em ciMalotes/inventario.\n\n";
    }

    echo "=== FIM (remova ?diag_displays apos conferir) ===\n";
    exit;
}

// === v2.0.3 #7: postos marcados como "adiantado" (ja despachados em ofício anterior) ===
// Servem para destacar a linha no PRÓXIMO ofício (cor diferente + aviso) para que o
// operador remova o posto antes de apropriar os lacres.
$postos_adiantados = array();
try {
    $pdo_controle->exec("CREATE TABLE IF NOT EXISTS ciDespachoAdiantado (
        id INT(11) NOT NULL AUTO_INCREMENT,
        posto VARCHAR(10) NOT NULL,
    ...
    // So destaca como "ja despachado" os postos cujo adiantamento tem data_producao
    // dentro das datas do filtro atual (no dia seguinte, ao filtrar a producao daquele
    // dia, o lote adiantado reaparece em vermelho para ser removido da grade).
    $datasAdYmd = array();
    if (isset($datas_filtro) && is_array($datas_filtro)) {
        foreach ($datas_filtro as $dfAd) {
            $dfAd = trim((string)$dfAd);
            if ($dfAd === '') continue;
            $dtAd = DateTime::createFromFormat('d-m-Y', $dfAd);
            if ($dtAd) { $datasAdYmd[] = $dtAd->format('Y-m-d'); }
        }
    }
    if (!empty($datasAdYmd)) {
        $phAd = implode(',', array_fill(0, count($datasAdYmd), '?'));
        $stAd = $pdo_controle->prepare("SELECT DISTINCT posto FROM ciDespachoAdiantado WHERE data_producao IN ($phAd)");
        $stAd->execute($datasAdYmd);
        while ($rAd = $stAd->fetch(PDO::FETCH_ASSOC)) {
            $codAd = str_pad(preg_replace('/\D/', '', (string)$rAd['posto']), 3, '0', STR_PAD_LEFT);
            if ($codAd !== '' && $codAd !== '000') { $postos_adiantados[$codAd] = true; }
        }
    }
} catch (Exception $e) { /* tolera */ }

// v2.0.3 #2 (REMOVIDO): a inclusao automatica de postos da "estante" quando o display
// voltava foi retirada — o display voltar NAO implica adiantar/incluir o posto. Adiantar
// um posto so acontece quando o posto pede (ver botao "Adiantar lote"). O estoque de display
// continua exibido por posto (badge), sem injetar linhas fora do filtro de datas.
...
if (!function_exists('lacresBadgesStatus')) {
    function lacresBadgesStatus($status, $usaDisplay = true, $mostrarDisplay = true) {
        if (!$status) return '';
        $html = '';
        // Info de display so para postos que usam display Correios.
        // $mostrarDisplay=false suprime a info de estoque (usado p/ nao repetir
        // a contagem da Central em cada linha — ela e a mesma para todos).
        if ($usaDisplay && $mostrarDisplay) {
            ...
            $present = isset($status['estoque_presente']) ? (int)$status['estoque_presente'] : 0;
            $local = ($disp !== null && $disp > 0) ? $disp : 0;
            // Universo p/ exibir "de N cadastrados": nao pode ser menor que local+transito,
            // pois o inventario/retorno pode revelar displays fora do cadastroMalotes.
            $totalExib = ($total !== null) ? $total : 0;
            if (($local + (int)$trans) > $totalExib) { $totalExib = $local + (int)$trans; }
            $conhecido = ($total !== null && $total > 0) || ($trans !== null && $trans > 0) || ($present > 0) || ($local > 0);
            ...
            if (isset($status['display']) && $status['display'] === 'fora') {
                ...
                if ($conhecido) {
                    $titulo = 'Nenhum display local (' . (int)$trans . ' em transito de ' . (int)$totalExib . ' cadastrados)';
                } else {
                ...
                $html .= '<span class="badge-status badge-display-fora badge-display-toggle nao-imprimir" title="' . htmlspecialchars($titulo, ENT_QUOTES, 'UTF-8') . '">' . $rotulo . '</span>';
            } else if ($conhecido) {
                // Ha display local disponivel => badge informativo com a contagem
                $titulo = $local . ' display(s) local, ' . (int)$trans . ' em transito de ' . (int)$totalExib . ' cadastrados';
                $rotulo = '&#128230; ' . $local . ' local &middot; ' . (int)$trans . ' em transito';
                $html .= '<span class="badge-status badge-display-info badge-display-toggle nao-imprimir" title="' . htmlspecialchars($titulo, ENT_QUOTES, 'UTF-8') . '">' . $rotulo . '</span>';
            } else {
                // v2.0.3: posto sem display cadastrado — contagem zero p/ aparecer p/ TODOS os postos
                $titulo = 'Nenhum display cadastrado para este posto';
                $rotulo = '&#128230; 0 local &middot; 0 em transito';
                $html .= '<span class="badge-status badge-display-none badge-display-toggle nao-imprimir" title="' . htmlspecialchars($titulo, ENT_QUOTES, 'UTF-8') . '">' . $rotulo . '</span>';
            }
        }
    ...
    foreach ($dados as $grupo => &$itens) {
        foreach ($itens as &$posto) {
            ...
            if ($vals) {
                ...
                // v2.0.4: restaurar tambem a ETIQUETA/display do snapshot (antes so lacres).
                // Linhas inseridas manualmente perdiam o display (input em branco) ao recarregar:
                // o restore so repunha os lacres e a etiqueta dependia de $_SESSION['etiquetas'],
                // que podia ser sobrescrito pelo load do oficio do BD antes do render.
                if (isset($vals['etiqueta_correios']) && (string)$vals['etiqueta_correios'] !== '') {
                    $posto['etiqueta_correios'] = $vals['etiqueta_correios'];
                    $_SESSION['etiquetas'][$codigo] = $vals['etiqueta_correios'];
                }
            }
        }
        unset($posto);
...
<head>
    ...
    <style>
        ...
        .modal-overlay {
            ...
            /* Acima do #popup-etiqueta-focal (z-index 10001): o aviso de display
               nao cadastrado / retorno nao pode ficar escondido atras do popup
               de informacoes da etiqueta da linha. */
            z-index: 10050;
            justify-content: center;
            align-items: center;
        }
        ...
        /* v2.0.3 #7: posto ja despachado (adiantado) — remover antes de apropriar lacres */
        tr.linha-adiantado > td { background: #fde2e1; }
        tr.linha-adiantado > td:first-child { box-shadow: inset 4px 0 0 0 #d9534f; }
        .badge-adiantado { display:inline-block; margin-left:6px; font-size:10px; font-weight:700; color:#fff; background:#d9534f; padding:1px 6px; border-radius:8px; }
        /* PT: lote ja participou de oficio anterior COM lacres — remover antes de gerar (evita duplicidade) */
        tr.linha-ja-oficio > td { background: #e1d5f7; }
        tr.linha-ja-oficio > td:first-child { box-shadow: inset 4px 0 0 0 #7e3ff2; }
        tr.linha-display-errado > td { background: #ffd6d6 !important; }
        tr.linha-display-errado > td:first-child { box-shadow: inset 4px 0 0 0 #d32f2f; }
        .badge-ja-oficio { display:inline-block; margin-left:6px; font-size:10px; font-weight:700; color:#fff; background:#7e3ff2; padding:1px 6px; border-radius:8px; }
        .lbl-adiantado input { vertical-align:middle; }
        ...
        .badge-display-none { background: #eceff1; color: #546e7a; border: 1px solid #b0bec5; }
        /* v2.0.3: estoque de display oculto por padrao; um unico switch global libera p/ TODOS os postos */
        .badge-display-toggle { display: none !important; }
        table.mostrar-displays .badge-display-toggle { display: inline-block !important; }
        /* v2.0.3: fonte menor p/ caber na coluna em monitor vertical (largura reduzida) */
        .badge-display-fora, .badge-display-info, .badge-display-none {
            font-size: 8px; padding: 1px 4px; margin: 0 2px 0 0; letter-spacing: -0.2px; border-radius: 8px;
        }
        /* Switch (empurrar para a direita p/ ativar) no cabecalho do grupo */
        .toggle-display { position: relative; display: inline-block; width: 30px; height: 16px; vertical-align: middle; margin-right: 8px; }
        .toggle-display input { opacity: 0; width: 0; height: 0; position: absolute; }
        .toggle-display .toggle-slider { position: absolute; cursor: pointer; top: 0; left: 0; right: 0; bottom: 0; background: #bbb; border-radius: 16px; transition: background .2s; }
        .toggle-display .toggle-slider:before { content: ""; position: absolute; height: 12px; width: 12px; left: 2px; bottom: 2px; background: #fff; border-radius: 50%; transition: transform .2s; }
        .toggle-display input:checked + .toggle-slider { background: #4caf50; }
        .toggle-display input:checked + .toggle-slider:before { transform: translateX(14px); }
        ...
        @media print {
            ...
            tr.linha-adiantado > td { background: transparent !important; }
            tr.linha-adiantado > td:first-child { box-shadow: none !important; }
            tr.linha-ja-oficio > td { background: transparent !important; }
            tr.linha-ja-oficio > td:first-child { box-shadow: none !important; }
            tr.linha-display-errado > td { background: transparent !important; }
            tr.linha-display-errado > td:first-child { box-shadow: none !important; }
...
<div class="version-info">v2.9.9</div>
    ...
    <form method="get" action="<?php echo $_SERVER['PHP_SELF'] ?>" id="formFiltroData" onsubmit="limparLacresPorRecalculo(); salvarEstadoEtiquetasCorreios();">
        ...
        <div style="margin:15px 0;padding:12px;background:#f8f9fa;border:1px solid #dee2e6;border-radius:4px;">
            ...
            <div style="margin-top:10px;">
                ...
                <div class="campo-calendario">
                    <label style="font-weight:bold;font-size:12px;color:#495057;">Tipo de Posto:</label><br>
                    <select name="filtro_grupo" id="filtro_grupo" title="Carregue somente os postos dos Correios ou somente do Poupa Tempo para a tela abrir mais rapido" style="width:190px;padding:6px 10px;border:1px solid #ced4da;border-radius:4px;background:#fff;">
                        <option value="" <?php echo $filtro_grupo === '' ? 'selected' : '' ?>>Todos (Correios + PT)</option>
                        <option value="correios" <?php echo $filtro_grupo === 'correios' ? 'selected' : '' ?>>Somente Correios</option>
                        <option value="poupatempo" <?php echo $filtro_grupo === 'poupatempo' ? 'selected' : '' ?>>Somente Poupa Tempo</option>
                    </select>
                </div>
        ...
        <div class="topo-formulario">
            ...
            <!-- v2.7.0 (B): numero do ultimo oficio Correios gerado -->
            <div style="display:inline-block; margin-left:15px; padding:8px 12px; background:#fff3cd; border:1px solid #ffc107; border-radius:4px; font-size:12px;">
                <strong style="color:#856404;">Último ofício Correios gerado:</strong>
                <strong style="color:#856404;"><?php echo $ultimo_despacho_correios > 0 ? '#' . (int)$ultimo_despacho_correios : '—'; ?></strong>
            </div>
...
<form method="post" id="formOficioCorreios" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF'] . (isset($_SERVER['QUERY_STRING']) && $_SERVER['QUERY_STRING'] !== '' ? '?' . $_SERVER['QUERY_STRING'] : ''), ENT_QUOTES, 'UTF-8'); ?>">
...
<?php if (!empty($poupaTempoPayload)): ?>
        ...
        <button type="button" class="btn btn-warning" id="btnGerarOficioPT" onclick="abrirOficioPoupaTempo();">Gerar Ofício Poupa Tempo</button>
        ...
        function abrirOficioPoupaTempo() {
            // NOVO: se houver lotes selecionados que ja estao em oficio anterior, alerta
            // ANTES de gerar; o usuario pode CANCELAR para excluir as linhas e nao duplicar.
            if (typeof window.confirmarLotesJaEmOficioAntesGerar === 'function') {
                if (!window.confirmarLotesJaEmOficioAntesGerar()) { return; }
            }
            if (typeof window.avisarLotesNaoConferidos === 'function') { window.avisarLotesNaoConferidos(); }
...
<script type="text/javascript">
// T4: aviso (nao bloqueia) de lotes do periodo aplicado ainda nao conferidos.
var LOTES_NAO_CONFERIDOS = <?php echo json_encode_legado_seguro($lotes_nao_conferidos, JSON_UNESCAPED_UNICODE); ?>;
function avisarLotesNaoConferidos() {
    try {
        if (!LOTES_NAO_CONFERIDOS || !LOTES_NAO_CONFERIDOS.length) { return true; }
        var total = LOTES_NAO_CONFERIDOS.length;
        var limite = 30;
        var linhas = [];
        for (var i = 0; i < LOTES_NAO_CONFERIDOS.length && i < limite; i++) {
            var it = LOTES_NAO_CONFERIDOS[i];
            var txt = 'Posto ' + it.posto + ' - lote ' + it.lote;
            if (it.quantidade) { txt += ' (' + it.quantidade + ' un)'; }
            linhas.push(txt);
        }
        if (total > limite) { linhas.push('... e mais ' + (total - limite) + ' lote(s).'); }
        var msg = '\u26A0 ATENCAO: ' + total + ' lote(s) do periodo aplicado AINDA NAO foram conferidos:\n\n'
                + linhas.join('\n')
                + '\n\nVerifique se nao esqueceu nenhum no carrinho/estante. O oficio sera gerado mesmo assim.';
        alert(msg);
    } catch (e) { /* aviso nunca trava a geracao */ }
    return true;
}
window.avisarLotesNaoConferidos = avisarLotesNaoConferidos;

// NOVO: lotes do periodo que JA participaram de oficio anterior COM lacres associados.
// (1) ao aplicar o filtro de datas, avisa em janela; as linhas ja vem pintadas (CSS
// .linha-ja-oficio). (2) antes de gerar o oficio, se alguma linha selecionada ainda
// estiver entre essas, alerta de novo e deixa o usuario CANCELAR para excluir antes
// (evita enviar lote duplicado num novo oficio).
var LOTES_JA_EM_OFICIO  = <?php echo json_encode_legado_seguro($lotes_ja_em_oficio, JSON_UNESCAPED_UNICODE); ?>;
var POSTOS_JA_EM_OFICIO = <?php echo json_encode_legado_seguro($postos_ja_em_oficio, JSON_UNESCAPED_UNICODE); ?>;
// v2.5.1: lista INFORMATIVA completa (todos os postos/lotes do periodo ja salvos em algum oficio).
var LOTES_PERIODO_INFO  = <?php echo json_encode_legado_seguro($lotes_periodo_info, JSON_UNESCAPED_UNICODE); ?>;

function _ptMontarLinhasJaEmOficio(lista, limite) {
    var linhas = [];
    ...
    if (lista.length > limite) { linhas.push('... e mais ' + (lista.length - limite) + ' lote(s).'); }
    return linhas;
}

// (1) Janela de aviso ao carregar a grade com filtro aplicado.
function avisarLotesJaEmOficio() {
    try {
        if (!LOTES_JA_EM_OFICIO || !LOTES_JA_EM_OFICIO.length) { return true; }
        var total = LOTES_JA_EM_OFICIO.length;
        var linhas = _ptMontarLinhasJaEmOficio(LOTES_JA_EM_OFICIO, 30);
        var msg = '\u26A0 ATENCAO: ' + total + ' lote(s) do periodo aplicado JA estao em OFICIO ANTERIOR (com lacres):\n\n'
                + linhas.join('\n')
                + '\n\nAs linhas desses postos estao DESTACADAS em ROXO na grade.\n'
                + 'Exclua essas linhas antes de gerar o oficio para NAO duplicar lotes.';
        alert(msg);
    } catch (e) { /* aviso nunca trava a pagina */ }
    return true;
}
window.avisarLotesJaEmOficio = avisarLotesJaEmOficio;

// (2) Confirmacao antes de gerar: olha SO os postos PT selecionados. Se algum estiver
// "ja em oficio", alerta e retorna false quando o usuario decide CANCELAR (para excluir).
function confirmarLotesJaEmOficioAntesGerar() {
    try {
        if (!POSTOS_JA_EM_OFICIO) { return true; }
        var temAlgum = false; var k;
        for (k in POSTOS_JA_EM_OFICIO) { if (POSTOS_JA_EM_OFICIO.hasOwnProperty(k)) { temAlgum = true; break; } }
        if (!temAlgum) { return true; }
        // Codigos PT que serao realmente enviados
        var selStr = (typeof window.coletarPostosSelecionadosPT === 'function') ? window.coletarPostosSelecionadosPT() : '';
        var selArr = selStr ? selStr.split(',') : [];
        var selMap = {};
        for (var i = 0; i < selArr.length; i++) {
            var c = ('000' + String(selArr[i]).replace(/\D+/g, '')).slice(-3);
            if (c !== '000') { selMap[c] = true; }
        }
        // Lotes ja-em-oficio cujos postos estao selecionados
        var conflitos = [];
        for (var j = 0; j < LOTES_JA_EM_OFICIO.length; j++) {
            var it = LOTES_JA_EM_OFICIO[j];
            if (selMap[it.posto]) { conflitos.push(it); }
        }
        if (!conflitos.length) { return true; }
        var linhas = _ptMontarLinhasJaEmOficio(conflitos, 30);
        var msg = '\u26A0 ATENCAO: voce esta gerando o oficio com ' + conflitos.length + ' lote(s) que JA estao em OFICIO ANTERIOR (com lacres):\n\n'
                + linhas.join('\n')
                + '\n\nGerar assim pode DUPLICAR esses lotes em dois oficios.\n\n'
                + 'Clique CANCELAR para voltar e excluir as linhas destacadas em ROXO,\n'
                + 'ou OK para gerar mesmo assim.';
        return confirm(msg);
    } catch (e) { return true; /* nunca travar por erro de JS */ }
}
window.confirmarLotesJaEmOficioAntesGerar = confirmarLotesJaEmOficioAntesGerar;

// (3) v2.5.1: notificacao CONSOLIDADA ao carregar a grade com filtro. Mostra em UMA janela:
// (a) o aviso de DUPLICIDADE real (LOTES_JA_EM_OFICIO — postos que estao em OUTRO oficio,
// destacados em ROXO) e (b) a lista INFORMATIVA completa (LOTES_PERIODO_INFO) de TODOS os
// postos/lotes do periodo que ja constam em algum oficio salvo (com lacres). Antes so existia
// (a), que esconde o proprio oficio canonico e por isso costumava listar so 1 posto.
function _ptNotificarOficioPeriodo() {
    try {
        var temDup  = !!(LOTES_JA_EM_OFICIO && LOTES_JA_EM_OFICIO.length);
        var temInfo = !!(LOTES_PERIODO_INFO && LOTES_PERIODO_INFO.length);
        if (!temDup && !temInfo) { return; }
        var partes = [];
        if (temDup) {
            var linhasDup = _ptMontarLinhasJaEmOficio(LOTES_JA_EM_OFICIO, 30);
            partes.push('\u26A0 ATENCAO: ' + LOTES_JA_EM_OFICIO.length + ' lote(s) do periodo estao em OUTRO oficio (risco de DUPLICAR):\n\n'
                + linhasDup.join('\n')
                + '\n\nAs linhas desses postos estao DESTACADAS em ROXO. Exclua-as antes de gerar para NAO duplicar lotes.');
        }
        if (temInfo) {
            var linhasInfo = _ptMontarLinhasJaEmOficio(LOTES_PERIODO_INFO, 40);
            partes.push('\u2139 INFORMATIVO: ' + LOTES_PERIODO_INFO.length + ' posto(s)/lote(s) do periodo JA constam em algum oficio salvo (com lacres):\n\n'
                + linhasInfo.join('\n')
                + '\n\n(Apenas informativo: regerar o MESMO oficio do periodo so o sobrescreve; nem todos precisam ser removidos.)');
        }
        alert(partes.join('\n\n----------------------------------------\n\n'));
    } catch (e) { /* aviso nunca trava a pagina */ }
}
window._ptNotificarOficioPeriodo = _ptNotificarOficioPeriodo;

// v2.8.2: REMOVIDO o pop-up automatico ao carregar a grade. Avisar que "postos do periodo
// ja constam em oficio anterior" e generico e ACONTECE quase sempre (postos do filtro atual
// costumam estar em oficios anteriores) — nao e impedimento para gerar o novo oficio. A
// deteccao real de DUPLICIDADE continua: (a) inline, com a linha em ROXO + selo "JA EM
// OFICIO" para os lotes do periodo que estao em OUTRO oficio; (b) a confirmacao
// `confirmarLotesJaEmOficioAntesGerar()` ANTES de gerar, que so dispara quando um posto
// REALMENTE selecionado tem lote duplicado. A funcao `_ptNotificarOficioPeriodo` segue
// definida (exposta em window) caso precise ser chamada manualmente, mas nao auto-dispara.
</script>
...
<div class="controles-selecao-grade nao-imprimir">
    ...
    <label class="toggle-display nao-imprimir" title="Mostrar/ocultar o estoque de displays (local &middot; em transito) de TODOS os postos" style="margin-left:14px;">
        <input type="checkbox" id="chkMostrarDisplays" onchange="toggleTodosDisplays(this)">
        <span class="toggle-slider"></span>
    </label>
    <span class="nao-imprimir" style="font-size:11px;color:#546e7a;vertical-align:middle;margin-left:5px;">Mostrar displays</span>
    <button type="button" class="btn-adiantar-lote nao-imprimir" onclick="abrirModalAdiantarLote()" title="Ler um lote produzido hoje que segue junto com esta producao (adiantado)" style="margin-left:14px;background:#d9534f;color:#fff;border:none;padding:6px 12px;border-radius:4px;cursor:pointer;font-size:12px;font-weight:bold;">⏩ Adiantar lote</button>
</div>
...
<?php // v2.8.2: resumo de postos/displays no topo da grade (atualiza ao incluir/remover postos). ?>
<div id="contagemDisplaysGrade" class="contagem-displays-grade nao-imprimir" style="display:none; margin:10px 0; padding:9px 13px; background:#eef4fb; border:1px solid #cfe0f1; border-radius:6px; font-size:13px; color:#1b3a57; line-height:1.6;">
    <strong>Resumo da grade:</strong>
    Correios: <strong id="cdg-correios">0</strong> postos/displays &middot;
    Poupa Tempo: <strong id="cdg-pt">0</strong> displays &middot;
    <strong>Total: <span id="cdg-total">0</span> displays</strong>
    <span class="cdg-info" title="Contagem de displays/malotes que serao usados. Regra: Posto 01 = 1; cada posto da Capital = 1; Central (qualquer quantidade de postos) = 1 (vao todos no mesmo malote/display); cada Regional = 1. Poupa Tempo conta display apenas nos postos 110 a 880 (os PT 05 a 80 nao usam display dos Correios). Atualiza ao incluir/remover postos." style="cursor:help; color:#5a7fa6; font-weight:bold; margin-left:4px;">&#9432;</span>
</div>
<script>
(function(){
    function _cdgNum(s){ return parseInt(String(s||'').replace(/\D+/g,''),10) || 0; }
    function _cdgCount(obj){ var n=0,k; for(k in obj){ if(obj.hasOwnProperty(k)) n++; } return n; }
    function _cdgSet(id,val){ var el=document.getElementById(id); if(!el) return; if('textContent' in el){ el.textContent=val; } else { el.innerText=val; } }
    function atualizarContagemDisplaysGrade(){
        var bar = document.getElementById('contagemDisplaysGrade');
        var cont = document.getElementById('gradeLacresContainer');
        if (!bar || !cont) return;
        var linhas = cont.querySelectorAll('tr[data-posto-codigo]');
        var capital = {}, regionais = {}, pt = {};
        var temCentral = false;
        for (var i=0;i<linhas.length;i++){
            var tr = linhas[i];
            var grupo = (tr.getAttribute('data-grupo')||'').toUpperCase();
            var codRaw = (tr.getAttribute('data-posto-codigo')||'').replace(/\D+/g,'');
            var codNum = _cdgNum(codRaw);
            if (grupo === 'POUPA TEMPO'){
                // PT capital 05..80 tem input mas NAO usa display; conta so 110..880.
                if (codNum >= 110 && codNum <= 880 && codRaw !== '') { pt[codRaw] = true; }
            } else if (grupo === 'CENTRAL IIPR'){
                temCentral = true; // toda a Central = 1 display (mesmo malote)
            } else if (grupo === 'REGIONAIS'){
                var reg = (tr.getAttribute('data-regional')||tr.getAttribute('data-regional-codigo')||'').replace(/\D+/g,'');
                var keyR = (reg !== '' && reg !== '0') ? reg : codRaw; // 1 por regional
                if (keyR !== '') regionais[keyR] = true;
            } else { // CAPITAL e demais grupos Correios: 1 por posto
                if (codRaw !== '') capital[codRaw] = true;
            }
        }
        var correios = _cdgCount(capital) + _cdgCount(regionais) + (temCentral ? 1 : 0);
        var ptCount = _cdgCount(pt);
        _cdgSet('cdg-correios', correios);
        _cdgSet('cdg-pt', ptCount);
        _cdgSet('cdg-total', correios + ptCount);
        bar.style.display = (correios + ptCount > 0) ? '' : 'none';
    }
    window.atualizarContagemDisplaysGrade = atualizarContagemDisplaysGrade;
    function _cdgInit(){
        atualizarContagemDisplaysGrade();
        var cont = document.getElementById('gradeLacresContainer');
        if (cont && typeof MutationObserver !== 'undefined'){
            try {
                var obs = new MutationObserver(function(){ atualizarContagemDisplaysGrade(); });
                obs.observe(cont, { childList:true, subtree:true });
            } catch(e){}
        }
    }
    if (document.addEventListener){
        if (document.readyState === 'loading'){ document.addEventListener('DOMContentLoaded', _cdgInit); }
        else { _cdgInit(); }
    } else if (window.attachEvent){
        window.attachEvent('onload', _cdgInit);
    } else {
        window.onload = _cdgInit;
    }
})();
</script>
<?php if (isset($_GET['diag_snapshot']) && isset($diagSnap)): ?>
<pre style="background:#fff8e1;border:2px solid #ff9800;padding:14px;margin:14px;font-size:12px;white-space:pre-wrap;line-height:1.5;">
<strong>DIAG SNAPSHOT (v2.9.6) — diagnostico da restauracao da grade por data</strong>
Restauracao rodou? <?php echo $diagSnap['ran'] ? 'SIM' : ('NAO (' . htmlspecialchars($diagSnap['reason'], ENT_QUOTES, 'UTF-8') . ')'); ?>

Datas do filtro (normalizadas): <?php echo htmlspecialchars($diagSnap['filtro'], ENT_QUOTES, 'UTF-8'); ?>

Oficio escolhido: <?php echo $diagSnap['match_id'] > 0 ? ('#' . (int)$diagSnap['match_id'] . ' (' . htmlspecialchars($diagSnap['match_tipo'], ENT_QUOTES, 'UTF-8') . ', datas=' . htmlspecialchars($diagSnap['match_datas'], ENT_QUOTES, 'UTF-8') . ')') : 'NENHUM'; ?>

Snapshot: <?php echo (int)$diagSnap['snap_len']; ?> chars | linhas no snapshot: <?php echo (int)$diagSnap['snap_linhas']; ?> | linhas MANUAIS no snapshot: <?php echo (int)$diagSnap['snap_manuais']; ?> | linhas manuais RECRIADAS: <?php echo (int)$diagSnap['criadas']; ?>

Oficios CORREIOS com snapshot (mais recentes):
<?php if (empty($diagSnap['candidatos'])): ?>  (NENHUM oficio tem snapshot gravado — gere/salve o oficio UMA vez nesta versao p/ criar a memoria)
<?php else: foreach ($diagSnap['candidatos'] as $cdd): ?>  #<?php echo (int)$cdd['id']; ?> | datas=<?php echo htmlspecialchars($cdd['datas'], ENT_QUOTES, 'UTF-8'); ?> | snap=<?php echo (int)$cdd['len']; ?> chars | contem o filtro? <?php echo $cdd['contem'] ? 'sim' : 'nao'; ?>
...
<?php foreach ($dados as $grupo => $itens): if (empty($itens)) continue; ?>
    <table id="tabela-<?php echo strtolower(str_replace(' ', '-', $grupo)) ?>" data-grupo="<?php echo htmlspecialchars($grupo, ENT_QUOTES, 'UTF-8') ?>">
        ...
        <tbody>
            ...
            ?>
            <?php $centralBadgeJaMostrado = false; ?>
            <?php foreach ($itensRenderizados as $key => $linhaRender): ?>
            <?php if ($linhaRender['tipo_linha'] === 'cabecalho'): ?>
            <tr class="subtitulo-pt-grid">
                <td colspan="5" style="text-align:left; font-weight:bold; background:#f2f2f2; padding:8px 10px;">&nbsp;<?php echo htmlspecialchars($linhaRender['titulo'], ENT_QUOTES, 'UTF-8'); ?><button type="button" class="btn-desmarcar-pt nao-imprimir" onclick="desmarcarGrupoPT(this)" title="Desmarcar todos os postos deste grupo" style="margin-left:12px; padding:2px 9px; font-size:11px; font-weight:600; background:#fff0f0; border:1px solid #d99; color:#a33; border-radius:4px; cursor:pointer;">Desmarcar todos</button></td>
            </tr>
            <?php continue; ?>
            <?php endif; ?>
            <?php $dado = $linhaRender['item']; ?>
            <?php
            // Postos da CENTRAL (grupo CENTRAL IIPR) ou que pertencem a ela (regional 999)
            // recebem o display da Central: usam o estoque 'central', nao o bucket proprio
            // (que so acumularia displays da Central em transito para aquele posto).
            // T3-fix: no grupo REGIONAIS o estoque de display e chaveado pelo NUMERO do posto
            // (campo 'regional', ex.: 700/950) e NAO pelo posto_codigo interno (ex.: 151/969).
            // O posto_codigo as vezes aparece como chave do estoque por artefato de movimento
            // (display rotulado errado em ciMalotes), com contagem parcial/zerada. Comprovado
            // via ?diag_displays= (700: posto_codigo=151 total=0 vs regional=700 total=12).
            // Central e Poupa Tempo mantem a chave posto_codigo (regra propria). Guard por isset
            // garante fail-safe. Regra unica via helper compartilhado com o ?diag_displays.
            $chave_status = lacresResolverChaveStatus($grupo, $dado['posto_codigo'], isset($dado['regional']) ? $dado['regional'] : '', $status_postos);
            $st_linha = isset($status_postos[$chave_status]) ? $status_postos[$chave_status] : null;
            ...
            // v2.0.3 #7: destaque de linha do posto marcado como ja despachado (adiantado)
            $cod_pad_linha = str_pad(preg_replace('/\D/', '', (string)$dado['posto_codigo']), 3, '0', STR_PAD_LEFT);
            $linha_adiantada = isset($postos_adiantados) && isset($postos_adiantados[$cod_pad_linha]);
            if ($linha_adiantada) { $classes_linha[] = 'linha-adiantado'; }
            // NOVO: posto PT cujo(s) lote(s) do periodo JA estao em oficio anterior com lacres
            $linha_ja_oficio = ($grupo === 'POUPA TEMPO') && isset($postos_ja_em_oficio[$cod_pad_linha]);
            if ($linha_ja_oficio) { $classes_linha[] = 'linha-ja-oficio'; }
            $attr_classe = !empty($classes_linha) ? ' class="' . implode(' ', $classes_linha) . '"' : '';
            ?>
            <tr data-posto-codigo="<?php echo $dado['posto_codigo'] ?>" data-grupo="<?php echo $grupo ?>" data-regional="<?php echo isset($dado['regional']) ? htmlspecialchars($dado['regional'], ENT_QUOTES, 'UTF-8') : '0' ?>" data-regional-codigo="<?php echo isset($dado['regional']) ? htmlspecialchars($dado['regional'], ENT_QUOTES, 'UTF-8') : '0' ?>" data-posto-real="<?php echo isset($dado['posto_real']) ? htmlspecialchars($dado['posto_real'], ENT_QUOTES, 'UTF-8') : '' ?>" data-posto-nome="<?php echo isset($dado['posto_nome']) ? htmlspecialchars($dado['posto_nome'], ENT_QUOTES, 'UTF-8') : '' ?>" data-linha-inserida="<?php echo !empty($dado['manual_inserido']) ? '1' : '0' ?>" data-ja-oficio="<?php echo $linha_ja_oficio ? '1' : '0' ?>"<?php echo $attr_classe; ?><?php if ($grupo === 'CENTRAL IIPR'): ?> data-central-index="<?php echo $key ?>"<?php endif; ?>>
                <td class="acoes-cell">
                    <?php if ($grupo === 'POUPA TEMPO'): ?>
                    ...
                    <?php
                    // v2.0.3: badge de estoque aparece p/ TODOS os postos (mesmo sem display
                    // cadastrado -> 0 local) quando o switch global e ligado. Central: so na 1a linha.
                    $mostrarDisplayLinha = true;
                    if ($grupo === 'CENTRAL IIPR') {
                        if ($centralBadgeJaMostrado) { $mostrarDisplayLinha = false; }
                        else { $centralBadgeJaMostrado = true; }
                    }
                    $st_para_badge = $st_linha;
                    if ($st_para_badge === null) {
                        $st_para_badge = array(
                            'estoque_total' => 0, 'estoque_transito' => 0, 'estoque_disponivel' => 0,
                            'display' => '', 'display_data' => null,
                            'bloqueio' => null, 'restricao_tipo' => null, 'restricao_motivo' => null
                        );
                    }
                    $badges = lacresBadgesStatus($st_para_badge, $usa_display, $mostrarDisplayLinha);
                    if ($badges !== ''):
                    ?>
                    <span class="status-badges nao-imprimir"><?php echo $badges; ?></span>
                    <?php endif; ?>
                    <?php if (!empty($linha_adiantada)): ?>
                    <span class="badge-adiantado nao-imprimir" title="Este posto foi marcado como despachado adiantado. Remova esta linha antes de apropriar os lacres.">JA DESPACHADO — remover</span>
                    <?php endif; ?>
                    <?php if (!empty($linha_ja_oficio)): ?>
                    <span class="badge-ja-oficio nao-imprimir" title="Este posto tem lote(s) do periodo que JA estao em oficio anterior com lacres. Remova esta linha antes de gerar para nao duplicar.">JA EM OFICIO — remover</span>
                    <?php endif; ?>
                    <?php if ($grupo !== 'POUPA TEMPO'): ?>
...
<div class="modal-overlay" id="modal-inserir">
    <div class="modal-content">
        ...
        <form method="post" class="modal-form">
            ...
            <label for="novo_codigo">Numero do posto (ex: 808):</label>
            <input type="text" name="novo_codigo" id="novo_codigo" inputmode="numeric" placeholder="ex: 808">

            <label for="novo_nome">Nome do Posto:</label>
            <input type="text" name="novo_nome" id="novo_nome" required>
            
            <div class="texto-ajuda">Informe o numero do posto para validar corretamente o display bipado. Lacres e etiqueta devem ser preenchidos na linha apos inserir o posto.</div>
...
<!-- #6: janela para cadastrar display nao registrado no cadastroMalotes -->
<div class="modal-overlay" id="modal-cadastrar-display">
    <div class="modal-content">
        <h3 class="modal-title">Display nao cadastrado</h3>
        <div class="modal-form">
            <p style="font-size:12px;color:#555;margin:0 0 8px;">
                Esta etiqueta nao esta no cadastroMalotes. Informe o posto para cadastra-la
                e manter o controle dos displays.
            </p>
            <input type="hidden" id="cd_leitura">
            <label>Etiqueta:</label>
            <input type="text" id="cd_leitura_show" readonly>
            <label>CEP:</label>
            <input type="text" id="cd_cep" readonly>
            <label>Sequencial:</label>
            <input type="text" id="cd_seq" readonly>
            <label>Posto:</label>
            <input type="text" id="cd_posto" placeholder="Ex.: 014" autocomplete="off">
            <div class="modal-buttons">
                <button type="button" class="modal-btn modal-btn-cancel" onclick="fecharModalCadastrarDisplay()">Agora nao</button>
                <button type="button" class="modal-btn modal-btn-save" onclick="salvarCadastroDisplay()">Cadastrar</button>
            </div>
        </div>
    </div>
</div>

<!-- Adiantar lote: ler codigo do lote (19 digitos), informar data de producao e executor -->
<div class="modal-overlay" id="modal-adiantar-lote">
    <div class="modal-content">
        <h3 class="modal-title">Adiantar lote (segue hoje)</h3>
        <div class="modal-form">
            <p style="font-size:12px;color:#555;margin:0 0 8px;">
                Leia o codigo de barras do lote (19 digitos) produzido HOJE que seguira junto
                com a producao deste oficio. Informe a data de producao e o executor.
                No proximo oficio desta producao a linha do posto aparecera como JA DESPACHADO.
            </p>
            <label>Codigo do lote (19 digitos):</label>
            <input type="text" id="ad_codbar" placeholder="Bipe o codigo do lote" autocomplete="off" inputmode="numeric" oninput="_atualizarResumoAdiantar()">
            <div id="ad_resumo" style="font-size:12px;color:#1976d2;margin:4px 0;min-height:16px;"></div>
            <label>Data de producao:</label>
            <input type="date" id="ad_data_producao">
            <label>Executor:</label>
            <input type="text" id="ad_executor" placeholder="Nome de quem despachou" autocomplete="off" value="<?php echo htmlspecialchars($responsavel) ?>">
            <div class="modal-buttons">
                <button type="button" class="modal-btn modal-btn-cancel" onclick="fecharModalAdiantarLote()">Cancelar</button>
                <button type="button" class="modal-btn modal-btn-save" onclick="salvarAdiantarLote()">Gravar adiantamento</button>
            </div>
        </div>
    </div>
</div>

<!-- #5: janela para registrar retorno (tipo=2) de display em transito -->
<div class="modal-overlay" id="modal-retorno-display">
    <div class="modal-content">
        <h3 class="modal-title">Display em transito</h3>
        <div class="modal-form">
            <p style="font-size:12px;color:#555;margin:0 0 8px;">
                Esta etiqueta saiu (tipo 1) e ainda nao teve o retorno registrado.
                Registre o retorno para manter a consistencia antes de continuar a expedicao.
            </p>
            <input type="hidden" id="rt_leitura">
            <label>Etiqueta:</label>
            <input type="text" id="rt_leitura_show" readonly>
            <label>Responsavel:</label>
            <input type="text" id="rt_responsavel" value="<?php echo htmlspecialchars($responsavel) ?>" autocomplete="off">
            <div class="modal-buttons">
                <button type="button" class="modal-btn modal-btn-cancel" onclick="fecharModalRetorno()">Continuar sem registrar</button>
                <button type="button" class="modal-btn modal-btn-save" onclick="salvarRetornoDisplay()">Registrar retorno</button>
            </div>
        </div>
    </div>
</div>
...
function prepararLacresCorreiosParaSubmit(form) {
    ...
    for (var r=0;r<rows.length;r++){
        ...
        // v2.9.5: Capturar nome/numero real + flag manual p/ poder RECONSTRUIR a linha
        // ao restaurar o oficio por data (linhas manuais somem da sessao no dia seguinte).
        var postoNome = tr.getAttribute('data-posto-nome') || '';
        var postoReal = tr.getAttribute('data-posto-real') || '';
        var manualInserido = (tr.getAttribute('data-linha-inserida') === '1') ? 1 : 0;
        ...
        snapshot.push({
            ...
            manual_inserido: manualInserido,
            lacre_iipr: valI,
            lacre_correios: valC,
...
function salvarEstadoEtiquetasCorreios() {
    ...
    for (var r = 0; r < rows.length; r++) {
        ...
        // v2.5.1: + lacre IIPR do Poupa Tempo (lacre_pt)
        ...
        var inpLacrePt = tr.querySelector('input[name^="lacre_pt"], input.lacre-pt, input[data-tipo="lacre-pt"]');
        
        var valI = inpIIPR ? String(inpIIPR.value || '').trim() : '';
        var valC = inpCorr ? String(inpCorr.value || '').trim() : '';
        var valE = inpEtiq ? String(inpEtiq.value || '').trim() : '';
        var valLP = inpLacrePt ? String(inpLacrePt.value || '').trim() : '';
...
function restaurarEstadoTemporarioLacres() {
    ...
    for (var r = 0; r < rows.length; r++) {
        ...
        if (inpLacrePt && valor.lacre_pt) inpLacrePt.value = valor.lacre_pt;
    }
...
function gerarSnapshotLacres() {
    ...
    for (var r = 0; r < rows.length; r++) {
        ...
        mapa[regionalCodigo + '|' + postoCodigo] = {
            ...
            lacre_pt: inpLacrePt ? String(inpLacrePt.value || '').trim() : ''
        };
    }
    return mapa;
}
...
function restaurarEstadoEtiquetasCorreios() {
    ...
    for (var r = 0; r < rows.length; r++) {
        ...
        if (valor) {
            ...
            if (inpLacrePt && valor.lacre_pt) {
                inpLacrePt.value = valor.lacre_pt;
            }
        }
    }
}
...
function preencherInputsParaImpressao() {
    ...
    for (var r = 0; r < rows.length; r++) {
        ...
        if (inpLacrePt && inpLacrePt.value) { inpLacrePt.setAttribute('value', inpLacrePt.value); }
    }
}
...
function gravarEImprimirCorreios() {
    ...
    if (typeof window.avisarLotesNaoConferidos === 'function') { window.avisarLotesNaoConferidos(); }
    if (typeof avisarDespachoDataFutura === 'function' && !avisarDespachoDataFutura()) { return; }
    ...
    var prosseguirGravar = function() {
        prepararLacresCorreiosParaSubmit(form);
        form.submit();
    };
    if (typeof window.validarEtiquetasAntesGravar === 'function') {
        window.validarEtiquetasAntesGravar(prosseguirGravar);
    } else {
        prosseguirGravar(); // FAIL-OPEN: validacao indisponivel nao trava a gravacao
    }
}
...
// v2.0.3 #7: datas da producao em tela (d-m-Y) p/ avisar despacho de data de hoje/futura
var DATAS_PRODUCAO_GRADE = <?php echo json_encode(array_values($datas_filtro)); ?>;
function avisarDespachoDataFutura() {
    try {
        if (!DATAS_PRODUCAO_GRADE || !DATAS_PRODUCAO_GRADE.length) return true;
        var hoje = new Date(); hoje.setHours(0, 0, 0, 0);
        var achou = false, exemplo = '';
        for (var i = 0; i < DATAS_PRODUCAO_GRADE.length; i++) {
            var s = String(DATAS_PRODUCAO_GRADE[i] || '').replace(/\s/g, '');
            var m = s.match(/^(\d{2})-(\d{2})-(\d{4})$/);
            if (!m) continue;
            var d = new Date(parseInt(m[3], 10), parseInt(m[2], 10) - 1, parseInt(m[1], 10));
            d.setHours(0, 0, 0, 0);
            if (d.getTime() >= hoje.getTime()) { achou = true; exemplo = s; break; }
        }
        if (achou) {
            return confirm('ATENCAO: voce esta despachando producao de data de HOJE ou FUTURA (' + exemplo + ').\n\n' +
                'Normalmente despacha-se producao de dias anteriores. Deseja continuar mesmo assim?');
        }
    } catch (e) { /* ignore */ }
    return true;
}
...
// Extrai os numeros de um campo de lacre que pode conter FAIXA/LISTA num mesmo input
// (ex.: "12430-12431", "12430/12431", "12430, 12431", "12430 12431").
// Sem isso, remover os nao-digitos juntaria "12430-12431" em "1243012431" (bug do aviso).
function lacreNumeros(valor) {
    var nums = [];
    var partes = String(valor == null ? '' : valor).split(/[^0-9]+/);
...
// Primeiro lacre do campo (inicio da faixa) -> usado p/ checar se DA continuidade ao anterior.
function lacrePrimeiroNumero(valor) {
    var nums = lacreNumeros(valor);
    return nums.length ? nums[0] : NaN;
}
// Maior lacre do campo (fim da faixa) -> usado como base p/ o proximo lacre da grade.
function lacreMaiorNumero(valor) {
    var nums = lacreNumeros(valor);
...
// v2.0.3 #8: maior lacre IIPR ja preenchido na(s) tabela(s) da grade informada(s)
function maxLacreIiprNasTabelas(ids) {
    var max = 0;
    for (var k = 0; k < ids.length; k++) {
        var tb = document.getElementById(ids[k]);
        if (!tb) continue;
        var ins = tb.querySelectorAll('input[name^="lacre_iipr"]');
        for (var j = 0; j < ins.length; j++) {
            var v = lacreMaiorNumero(ins[j].value);
...
// v2.0.3 #8: avisa (sem bloquear) quando o 1o lacre de um grupo NAO e a sequencia do
// ultimo lacre usado: do ultimo oficio (ULTIMO_LACRE_IIPR) ou, ao passar de Capital
// para Central/Regionais (metropolitana), do ultimo lacre ja apropriado na grade.
function avisarSequenciaLacreInicial(el) {
    try {
        if (!el) return;
        var n = lacrePrimeiroNumero(el.value);
        if (isNaN(n) || n < 1) return;
        var esperado = 0;
        var origem = '';
        if (el.id === 'lacre_regionais_input') {
            var m = maxLacreIiprNasTabelas(['tabela-capital', 'tabela-central-iipr']);
            if (m > 0) { esperado = m + 1; origem = 'ultimo lacre da Capital/Central na grade (' + m + ')'; }
        } else if (el.id === 'lacre_central_input') {
            var mc = maxLacreIiprNasTabelas(['tabela-capital']);
            if (mc > 0) { esperado = mc + 1; origem = 'ultimo lacre da Capital na grade (' + mc + ')'; }
        }
        if (esperado === 0 && ULTIMO_LACRE_IIPR > 0) {
            esperado = ULTIMO_LACRE_IIPR + 1;
            origem = 'ultimo lacre IIPR ja utilizado (' + ULTIMO_LACRE_IIPR + ')';
        }
        if (esperado > 0 && n !== esperado) {
            // reuso (n <= ultimo do oficio) ja e avisado por avisarLacreReutilizado
            if (origem.indexOf('ja utilizado') >= 0 && n <= ULTIMO_LACRE_IIPR) return;
            alert('AVISO: o lacre ' + n + ' NAO e a sequencia esperada (' + esperado + '),\n' +
                  'com base no ' + origem + '.\n\n' +
                  '(Apenas um aviso — voce pode continuar.)');
        }
    } catch (e) { /* ignore */ }
}

// Maior lacre (IIPR ou Correios) ja preenchido nas LINHAS da grade ACIMA do input dado.
// Usa a ordem do documento (querySelectorAll) p/ refletir a sequencia Capital->Central->Regionais.
function maxLacreLinhasAcima(el) {
    var max = 0;
    try {
        var todos = document.querySelectorAll('input[data-tipo="iipr"], input[data-tipo="correios"]');
        for (var i = 0; i < todos.length; i++) {
            if (todos[i] === el) break;
            var v = lacreMaiorNumero(todos[i].value);
            if (!isNaN(v) && v > max) { max = v; }
        }
    } catch (e) { /* ignore */ }
    return max;
...
// Aviso POR LINHA ao confirmar um lacre:
//  - se ja existe lacre apropriado acima e o valor NAO da continuidade (max+1) -> janela;
//  - se e o 1o lacre da grade e e MENOR que o ultimo lacre usado em oficio anterior -> janela.
// Apenas avisa (nao bloqueia). Pula a coluna Correios da Central (regra de lacre diferente).
function avisarLacreLinha(el) {
    try {
        if (!el) return;
        var tipo = el.getAttribute('data-tipo');
        if (tipo !== 'iipr' && tipo !== 'correios') return;
        if (el.className && el.className.indexOf('central-correios') >= 0) return;
        var n = lacrePrimeiroNumero(el.value);
        if (isNaN(n) || n < 1) return;
        var maxAcima = maxLacreLinhasAcima(el);
        if (maxAcima > 0) {
            var esperado = maxAcima + 1;
            if (n !== esperado) {
                alert('AVISO de continuidade: o lacre ' + n + ' NAO da sequencia ao ultimo\n' +
                      'lacre ja apropriado na grade (' + maxAcima + '). Esperado: ' + esperado + '.\n\n' +
                      '(Apenas um aviso — feche para continuar.)');
            }
        } else {
            var ult = Math.max(ULTIMO_LACRE_IIPR || 0, ULTIMO_LACRE_CORREIOS || 0);
            if (ult > 0 && n < ult) {
                alert('AVISO: o lacre ' + n + ' e MENOR que o ultimo lacre ja\n' +
                      'utilizado em oficio anterior (' + ult + ').\n\n' +
                      '(Apenas um aviso — feche para continuar.)');
            }
        }
    } catch (e) { /* ignore */ }
...
function atribuirLacresSequencial() {
    ...
    // v2.0.3 #8: avisa (sem bloquear) quando o 1o lacre nao continua o ultimo oficio
    if (ULTIMO_LACRE_IIPR > 0 && numeroInicial > (ULTIMO_LACRE_IIPR + 1)) {
        alert('AVISO: o primeiro lacre (' + numeroInicial + ') NAO e a sequencia do ultimo\n' +
              'lacre IIPR ja utilizado (' + ULTIMO_LACRE_IIPR + '). Esperado: ' + (ULTIMO_LACRE_IIPR + 1) + '.\n\n' +
              '(Apenas um aviso — a atribuicao continua.)');
    }
...
function inicializarMonitoramentoAlteracoes() {
    ...
    // Aviso de continuidade / lacre menor POR LINHA — dispara na confirmacao (change),
    // nao em blur por clique fora (change so ocorre quando o valor mudou).
    try {
        var lacresLinha = document.querySelectorAll('input[data-tipo="iipr"], input[data-tipo="correios"]');
        for (var li = 0; li < lacresLinha.length; li++) {
            lacresLinha[li].addEventListener('change', function(ev) {
                var el = ev.target || ev.srcElement;
                if (typeof avisarLacreLinha === 'function') { avisarLacreLinha(el); }
            });
        }
    } catch (e) { /* ignore */ }
    ...
    try {
        ...
        var avisarReusoNoBlur = function(ev) {
            try {
                var el = ev.target || ev.srcElement;
                if (el && el.value) { avisarLacreReutilizado(el.value); avisarSequenciaLacreInicial(el); }
            } catch (e) { /* ignore */ }
        };
...
// Switch no cabecalho do grupo: mostra/oculta o estoque de display (local / em transito)
function toggleDisplaysGrupo(chk) {
    var tabela = chk;
    while (tabela && tabela.tagName !== 'TABLE') { tabela = tabela.parentNode; }
    if (!tabela) return;
    var cls = tabela.className || '';
    if (chk.checked) {
        if (cls.indexOf('mostrar-displays') < 0) { tabela.className = (cls + ' mostrar-displays').replace(/^\s+/, ''); }
    } else {
        tabela.className = cls.replace(/\s*mostrar-displays/g, '');
    }
}

// v2.0.3: switch global unico — mostra/oculta o estoque de display de TODOS os postos
function toggleTodosDisplays(chk) {
    var cont = document.getElementById('gradeLacresContainer');
    if (!cont) return;
    var tabelas = cont.getElementsByTagName('table');
    for (var i = 0; i < tabelas.length; i++) {
        var t = tabelas[i];
        var cls = t.className || '';
        if (chk.checked) {
            if (cls.indexOf('mostrar-displays') < 0) { t.className = (cls + ' mostrar-displays').replace(/^\s+/, ''); }
        } else {
            t.className = cls.replace(/\s*mostrar-displays/g, '');
        }
    }
}
...
// Desmarca todos os checkboxes (.pt-selecionar) do subgrupo (CAPITAL ou INTERIOR)
// a partir do botao no subtitulo: percorre as linhas seguintes ate o proximo
// subtitulo (tr.subtitulo-pt-grid) ou fim da tabela.
function desmarcarGrupoPT(btn) {
    var tr = btn;
    ...
    while (row) {
        if (row.nodeType === 1 && row.nodeName === 'TR') {
            ...
            if (cls.indexOf('subtitulo-pt-grid') !== -1) break;
            var chks = row.querySelectorAll ? row.querySelectorAll('.pt-selecionar') : [];
            for (var i = 0; i < chks.length; i++) {
                if (chks[i].checked) { chks[i].checked = false; n++; }
            }
        }
        row = row.nextSibling;
    }
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Item 2: numeracao continua (roll unico) quando ha SPLIT na central.
    // Cada bloco consome os IIPR dos seus postos + 1 numero de Lacre Correios.
    // Assim o 1o IIPR do bloco seguinte = ultimo Correios do bloco anterior + 1
    // (nunca repete o mesmo numero). Mantem 1 Correios por malote/bloco.
    function recomputarLacresCentralRoll(linhasCentral, faixas) {
        if (!linhasCentral || !linhasCentral.length || !faixas || !faixas.length) return;
        var primeira = linhasCentral[0] ? linhasCentral[0].querySelector('input[name^="lacre_iipr"]') : null;
        if (!primeira) return;
        var base = parseInt(String(primeira.value || '').replace(/\D/g, ''), 10);
        if (isNaN(base) || base < 1) return; // sem lacre inicial valido: nao mexe
        var contador = base;
        for (var f = 0; f < faixas.length; f++) {
            var ini = faixas[f].start;
            var fim = faixas[f].end;
            var r, linha, inp;
            // IIPR: um numero crescente por posto
            for (r = ini; r <= fim; r++) {
                linha = linhasCentral[r];
                if (!linha) continue;
                inp = linha.querySelector('input[name^="lacre_iipr"]');
                if (inp && !inp.disabled) {
                    inp.value = contador;
                    inp.setAttribute('value', String(contador));
                }
                contador++;
            }
            // Lacre Correios do bloco: um unico numero (proximo do roll)
            var correiosBloco = contador;
            contador++;
            for (r = ini; r <= fim; r++) {
                linha = linhasCentral[r];
                if (!linha) continue;
                inp = linha.querySelector('input.central-correios');
                if (inp) {
                    inp.value = correiosBloco;
                    inp.setAttribute('value', String(correiosBloco));
                }
            }
        }
    }
    ...
    function atualizarEstadoSplitCentral() {
        ...
        // Item 2: renumera IIPR/Correios como roll continuo antes de travar os campos
        recomputarLacresCentralRoll(linhasCentral, faixas);
        for (var f = 0; f < faixas.length; f++) {
    ...
    /* v2.3.2: destaca/limpa a linha cujo display lido e de outro posto */
    function marcarLinhaDisplayErrado(tr, ativo) {
        if (!tr) return;
        if (tr.classList) {
            if (ativo) { tr.classList.add('linha-display-errado'); }
            else { tr.classList.remove('linha-display-errado'); }
            return;
        }
        var cls = ' ' + (tr.className || '') + ' ';
        var tem = cls.indexOf(' linha-display-errado ') !== -1;
        if (ativo && !tem) { tr.className = (tr.className ? tr.className + ' ' : '') + 'linha-display-errado'; }
        else if (!ativo && tem) { tr.className = cls.replace(' linha-display-errado ', ' ').replace(/^\s+|\s+$/g, ''); }
    }
    ...
    function verificarDisplayPosto(inp) {
        ...
        // Linhas inseridas manualmente tem codigo interno "M..." (unico); o numero
        // real do posto fica em data-posto-real e e' o que deve casar com o display.
        var postoRealLinha = tr ? (tr.getAttribute('data-posto-real') || '').replace(/^p_/, '') : '';
        if (!postoLinha && !regionalLinha && !postoRealLinha) return;
        ...
        xhr.onreadystatechange = function() {
            ...
            try {
                ...
                if (res.ok && res.posto !== null && res.posto !== undefined) {
                    ...
                    var postoRealLn = String(postoRealLinha).replace(/\s+/g, '').replace(/^0+/, '');
                    ...
                    // Linhas manuais casam pelo numero real do posto (data-posto-real).
                    var bateu = (postoEtiq === postoLn) || (regionalLn !== '' && postoEtiq === regionalLn) || (postoRealLn !== '' && postoEtiq === postoRealLn);
                    // Postos da CENTRAL (grupo CENTRAL IIPR ou regional 999) recebem displays
                    // da Central; uma etiqueta cujo posto e' textual (ex.: 'central') e' valida
                    // para essas linhas. Ja uma etiqueta de posto numerico diferente dispara aviso.
                    // Etiqueta da CENTRAL = posto textual 'central'. NAO tratar qualquer
                    // valor nao numerico como central (postos 250*/450*/etc. sao reais).
                    var grupoLinha = tr ? (tr.getAttribute('data-grupo') || '') : '';
                    var linhaEhCentral = (grupoLinha === 'CENTRAL IIPR') || (regionalLn === '999');
                    var etiqEhCentral = /central/i.test(String(res.posto));
                    if (!bateu && linhaEhCentral && etiqEhCentral) { bateu = true; }
                    if (!bateu) {
                        avisoEl.textContent = '\u26a0 Display de OUTRO posto (posto ' + res.posto + ')';
                        avisoEl.style.display = 'block';
                        avisoEl.style.background = '#f8d7da';
                        avisoEl.style.borderColor = '#d32f2f';
                        avisoEl.style.color = '#a01818';
                        /* v2.9.9: apenas destaca a linha; NAO toca o mp3 "pacote de outra regional" (era confuso p/ display) */
                        marcarLinhaDisplayErrado(tr, true);
                        /* v2.9.9: aviso UNICO — somente o audio "display de outro posto" */
                        falarTexto('display de outro posto');
                    } else {
                        avisoEl.style.display = 'none';
                        avisoEl.style.background = '#fff3cd';
                        avisoEl.style.borderColor = '#e0a800';
                        avisoEl.style.color = '#856404';
                        marcarLinhaDisplayErrado(tr, false);
                    }
                } else {
                    avisoEl.style.display = 'none';
                }
                // #6 / #5: janelas de cadastro de display e registro de retorno.
                // Dispara no maximo uma janela por valor escaneado e so quando
                // nenhuma outra janela ja esta aberta (nao atrapalha a bipagem).
                var jaTratou = inp.getAttribute('data-modal-val') === val;
                var algumaAberta = document.querySelector('.modal-overlay.active');
                if (!jaTratou && !algumaAberta) {
                    if (res.cadastrado === false && res.cep) {
                        inp.setAttribute('data-modal-val', val);
                        abrirModalCadastrarDisplay(res.leitura || val, res.cep, res.seq, res.posto);
                    } else if (res.em_transito === true && res.presente_inventario === false) {
                        inp.setAttribute('data-modal-val', val);
                        abrirModalRetornoDisplay(res.leitura || val);
                    }
                }
    ...
    // ===== Validacao pre-gravacao: avisa etiqueta nao cadastrada + oferece cadastro =====
    // Coleta as etiquetas Correios preenchidas (grade Correios/Regionais e Central)
    // que estao com 35 digitos, ignorando linhas ocultas/excluidas.
    function coletarEtiquetasPreenchidas() {
        var out = [];
        var inputs = document.querySelectorAll('input.etiqueta-validavel, input.central-etiqueta');
        for (var i = 0; i < inputs.length; i++) {
            var inp = inputs[i];
            var tr = inp;
            while (tr && tr.tagName !== 'TR') { tr = tr.parentNode; }
            if (tr && tr.style && tr.style.display === 'none') continue;
            var dig = (inp.value || '').replace(/\D/g, '');
            if (dig.length !== 35) continue;
            var posto = tr ? (tr.getAttribute('data-posto-codigo') || '').replace(/^p_/, '') : '';
            out.push({ leitura: dig, posto: posto });
        }
        return out;
    }

    // Roda a checagem em lote no servidor. Se houver etiquetas nao cadastradas,
    // pergunta se o usuario quer cadastra-las agora (abre as janelas) ou gravar
    // mesmo assim. Em qualquer falha de rede, segue o save (fail-open) para nao
    // travar o trabalho.
    function validarEtiquetasAntesGravar(onProceed) {
        var lista = coletarEtiquetasPreenchidas();
        if (!lista.length) { onProceed(); return; }
        var body = 'acao=verificar_etiquetas_lote';
        for (var i = 0; i < lista.length; i++) {
            body += '&leitura%5B%5D=' + encodeURIComponent(lista[i].leitura) +
                    '&posto%5B%5D=' + encodeURIComponent(lista[i].posto || '');
        }
        var xhr = new XMLHttpRequest();
        var done = false;
        xhr.open('POST', location.href, true);
        ...
        xhr.onreadystatechange = function() {
            ...
            try {
                ...
                if (r && r.ok && r.nao_cadastradas) { nc = r.nao_cadastradas; }
            } catch (e) { onProceed(); return; }
            if (!nc.length) { onProceed(); return; }
            var msg = 'As seguintes etiquetas NAO estao cadastradas em cadastroMalotes:\n\n';
            for (var j = 0; j < nc.length; j++) {
                var p = nc[j].posto_linha || nc[j].posto_sugerido || '?';
                msg += '  Posto ' + p + ': ' + nc[j].leitura + '\n';
            }
            msg += '\nOK = cadastrar agora (abre as janelas)\nCancelar = gravar mesmo assim';
            if (confirm(msg)) {
                iniciarFilaCadastro(nc);   // nao grava agora; usuario regrava depois
            } else {
                onProceed();
            }
        };
        xhr.onerror = function() { if (!done) { done = true; onProceed(); } };
        try { xhr.send(body); } catch (e) { if (!done) { done = true; onProceed(); } }
    }

    // Fila de cadastro: abre uma janela de cadastro por etiqueta nao cadastrada.
    var _filaCadastro = [];
    var _filaCadastroAtiva = false;
    function iniciarFilaCadastro(lista) {
        _filaCadastro = lista ? lista.slice() : [];
        _filaCadastroAtiva = true;
        processarProximoCadastro();
    }
    function processarProximoCadastro() {
        if (!_filaCadastro.length) {
            _filaCadastroAtiva = false;
            alert('Cadastro concluido. Clique em Gravar novamente para salvar o oficio.');
            return;
        }
        var it = _filaCadastro.shift();
        abrirModalCadastrarDisplay(it.leitura, it.cep, it.seq, it.posto_linha || it.posto_sugerido || '');
    }

    // ===== #6: janela para cadastrar display nao registrado =====
    function abrirModalCadastrarDisplay(leitura, cep, seq, posto) {
        var m = document.getElementById('modal-cadastrar-display');
        if (!m) return;
        // Esconde o popup de informacoes da etiqueta para o aviso aparecer na frente.
        if (typeof ocultarPopupEtiqueta === 'function') { try { ocultarPopupEtiqueta(); } catch(e){} }
        document.getElementById('cd_leitura').value = leitura || '';
        document.getElementById('cd_leitura_show').value = leitura || '';
        document.getElementById('cd_cep').value = cep || '';
        document.getElementById('cd_seq').value = seq || '';
        document.getElementById('cd_posto').value = posto || '';
        m.classList.add('active');
        var pi = document.getElementById('cd_posto'); if (pi) { try { pi.focus(); } catch(e){} }
    }
    function fecharModalCadastrarDisplay() {
        var m = document.getElementById('modal-cadastrar-display');
        if (m) m.classList.remove('active');
        // Fechar manualmente (cancelar/X) aborta a fila de cadastro pendente.
        _filaCadastroAtiva = false;
        _filaCadastro = [];
    }
    function salvarCadastroDisplay() {
        var leitura = (document.getElementById('cd_leitura').value || '').replace(/\D/g, '');
        var posto = (document.getElementById('cd_posto').value || '').replace(/\s/g, '');
        if (leitura.length !== 35) { alert('Etiqueta invalida.'); return; }
        if (!posto) { alert('Informe o posto.'); return; }
        var xhr = new XMLHttpRequest();
        xhr.open('POST', location.href, true);
        xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        xhr.onreadystatechange = function() {
            ...
            try {
                var r = JSON.parse(xhr.responseText);
                if (r.ok) {
                    if (_filaCadastroAtiva) {
                        // Em fila: esconde sem abortar e abre a proxima janela.
                        var m = document.getElementById('modal-cadastrar-display');
                        if (m) m.classList.remove('active');
                        processarProximoCadastro();
                    } else {
                        alert(r.jaExistia ? 'Display ja estava cadastrado.' : 'Display cadastrado com sucesso.');
                        fecharModalCadastrarDisplay();
                    }
                } else {
                    alert('Falha ao cadastrar: ' + (r.msg || 'erro'));
                }
            } catch(e) { alert('Falha ao cadastrar o display.'); }
        };
        xhr.send('acao=cadastrar_display&leitura=' + encodeURIComponent(leitura) +
                 '&posto=' + encodeURIComponent(posto));
    }

    // ===== #5: janela para registrar retorno (tipo=2) de display em transito =====
    function abrirModalRetornoDisplay(leitura) {
        var m = document.getElementById('modal-retorno-display');
        if (!m) return;
        // Esconde o popup de informacoes da etiqueta para o aviso aparecer na frente.
        if (typeof ocultarPopupEtiqueta === 'function') { try { ocultarPopupEtiqueta(); } catch(e){} }
        document.getElementById('rt_leitura').value = leitura || '';
        document.getElementById('rt_leitura_show').value = leitura || '';
        m.classList.add('active');
    }
    ...
    function salvarRetornoDisplay() {
        var leitura = (document.getElementById('rt_leitura').value || '').replace(/\D/g, '');
        var resp = (document.getElementById('rt_responsavel').value || '');
        if (leitura.length !== 35) { alert('Etiqueta invalida.'); return; }
        var xhr = new XMLHttpRequest();
        xhr.open('POST', location.href, true);
        xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        xhr.onreadystatechange = function() {
            ...
            try {
                var r = JSON.parse(xhr.responseText);
                if (r.ok) {
                    alert(r.jaExistia ? 'Retorno ja registrado hoje.' : 'Retorno registrado com sucesso.');
                    fecharModalRetorno();
                } else {
                    alert('Falha ao registrar retorno: ' + (r.msg || 'erro'));
                }
            } catch(e) { alert('Falha ao registrar o retorno.'); }
        };
        xhr.send('acao=registrar_retorno&leitura=' + encodeURIComponent(leitura) +
                 '&responsavel=' + encodeURIComponent(resp));
    }

    // ===== Adiantar lote: modal p/ ler o codigo do lote e gravar em ciDespachoAdiantado =====
    function abrirModalAdiantarLote() {
        var m = document.getElementById('modal-adiantar-lote');
        if (!m) return;
        if (typeof ocultarPopupEtiqueta === 'function') { try { ocultarPopupEtiqueta(); } catch(e){} }
        var c = document.getElementById('ad_codbar'); if (c) c.value = '';
        var r = document.getElementById('ad_resumo'); if (r) r.innerHTML = '';
        m.classList.add('active');
        if (c) { try { c.focus(); } catch(e){} }
    }
    function fecharModalAdiantarLote() {
        var m = document.getElementById('modal-adiantar-lote');
        if (m) m.classList.remove('active');
    }
    function _parseLoteAdiantar(cod) {
        cod = (cod || '').replace(/\D/g, '');
        if (cod.length < 19) return null;
        cod = cod.substr(cod.length - 19);
        return { lote: cod.substr(0, 8), regional: cod.substr(8, 3), posto: cod.substr(11, 3), qtd: parseInt(cod.substr(14, 5), 10) || 0 };
    }
    function _atualizarResumoAdiantar() {
        var el = document.getElementById('ad_resumo');
        if (!el) return;
        var info = _parseLoteAdiantar(document.getElementById('ad_codbar').value);
        if (info) {
            el.innerHTML = 'Posto <strong>' + info.posto + '</strong> &middot; Lote <strong>' + info.lote + '</strong> &middot; Qtd <strong>' + info.qtd + '</strong>';
        } else {
            el.innerHTML = '';
        }
    }
    function salvarAdiantarLote() {
        var cod = (document.getElementById('ad_codbar').value || '').replace(/\D/g, '');
        var info = _parseLoteAdiantar(cod);
        if (!info) { alert('Codigo do lote invalido. Bipe os 19 digitos.'); return; }
        var dataProd = document.getElementById('ad_data_producao').value || '';
        var executor = document.getElementById('ad_executor').value || '';
        if (!dataProd) { alert('Informe a data de producao.'); return; }
        if (!executor) { alert('Informe o executor.'); return; }
        var xhr = new XMLHttpRequest();
        xhr.open('POST', location.href, true);
        xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        xhr.onreadystatechange = function() {
            ...
            try {
                var r = JSON.parse(xhr.responseText);
                if (r.ok) {
                    alert('Adiantamento gravado: posto ' + r.posto + ', lote ' + r.lote + ' (qtd ' + r.qtd + ').\n' +
                          'No proximo oficio desta producao a linha aparecera como JA DESPACHADO.');
                    fecharModalAdiantarLote();
                } else {
                    alert('Falha ao gravar: ' + (r.msg || 'erro'));
                }
            } catch(e) { alert('Falha ao gravar o adiantamento.'); }
        };
        xhr.send('acao=adiantar_lote&codbar=' + encodeURIComponent(cod) +
                 '&data_producao=' + encodeURIComponent(dataProd) +
                 '&executor=' + encodeURIComponent(executor));
    }

    // Exporta para escopo global: chamadas a partir dos botoes Gravar (fora desta closure)
    // e dos onclick inline dos modais precisam alcancar estas funcoes.
    window.abrirModalAdiantarLote = abrirModalAdiantarLote;
    window.fecharModalAdiantarLote = fecharModalAdiantarLote;
    window.salvarAdiantarLote = salvarAdiantarLote;
    window._atualizarResumoAdiantar = _atualizarResumoAdiantar;
    window.validarEtiquetasAntesGravar = validarEtiquetasAntesGravar;
    window.salvarCadastroDisplay = salvarCadastroDisplay;
    window.fecharModalCadastrarDisplay = fecharModalCadastrarDisplay;
    window.salvarRetornoDisplay = salvarRetornoDisplay;
    ...
    (function() {
        ...
        // v2.x: tambem validar "display de outro posto" no input da CENTRAL.
        // (central-etiqueta NAO tem a classe etiqueta-validavel de proposito, para ficar
        //  fora da checagem de duplicata — central pode compartilhar — mas precisa do aviso.)
        ligarValidacaoDisplay(document.querySelectorAll('input.central-etiqueta'));
        // v2.3.2: validar "display de outro posto" tambem nas linhas Poupa Tempo (etiqueta-pt)
        ligarValidacaoDisplay(document.querySelectorAll('input.etiqueta-pt'));
        // v2.3.2: campos de LACRE aceitam apenas numeros (bloqueia letras); preserva sequencias (- / , espaco).
        // Nao afeta os campos de ETIQUETA/display (classe etiqueta-barras), que sao alfanumericos.
        (function(){
            function ehLacre(el){
                if (!el || el.tagName !== 'INPUT' || !el.className || typeof el.className !== 'string') { return false; }
                return (' ' + el.className + ' ').indexOf(' lacre ') !== -1;
            }
            function limparLacre(el){ var v = el.value; if (v == null) { return; } var n = v.replace(/[^0-9\-\/,\s]/g, ''); if (n !== v) { el.value = n; } }
            function onEvtLacre(e){ var ev = e || window.event; var el = ev && (ev.target || ev.srcElement); if (ehLacre(el)) { limparLacre(el); } }
            if (document.addEventListener){ document.addEventListener('input', onEvtLacre, true); document.addEventListener('keyup', onEvtLacre, true); document.addEventListener('change', onEvtLacre, true); }
            else if (document.attachEvent){ document.attachEvent('onkeyup', onEvtLacre); document.attachEvent('onchange', onEvtLacre); }
        })();
                ...
                muts.forEach(function(m) {
                    m.addedNodes.forEach(function(n) {
                        if (n.querySelectorAll) {
                            ...
                            ligarValidacaoDisplay(n.querySelectorAll('input.central-etiqueta'));
                            ligarValidacaoDisplay(n.querySelectorAll('input.etiqueta-pt'));
                        }
                    });
                });
    ...
    for (var v = 0; v < etiquetasValidaveis.length; v++) {
        (function(inputEtiqueta) {
            ...
            inputEtiqueta.addEventListener('input', function() {
                ...
                if (valor.length >= 35) {
                    // v2.0.3: marca que o blur foi por leitura completa (scanner) p/ permitir avancar
                    this.setAttribute('data-auto-advance', '1');
            ...
            inputEtiqueta.addEventListener('blur', function() {
                ...
                // v2.0.3: so avanca p/ o proximo input quando a leitura foi completada (scanner/35 digitos);
                // clique fora do input NAO deve pular para o de baixo nem abrir o popup do proximo.
                var avancarAuto = this.getAttribute('data-auto-advance') === '1';
                this.removeAttribute('data-auto-advance');
                ...
                if (totalOcorrencias > 1) {
                    ...
                    if (manterDuplicada) {
                        ...
                        if (avancarAuto) { focarProximaEtiqueta(this); }
                    } else {
                    ...
                    var alertaDiv = document.getElementById('alerta-' + indice);
                    if (alertaDiv) { alertaDiv.style.display = 'none'; alertaDiv.textContent = ''; }
                    // v8.4: Se aceito, avançar para o próximo input de etiqueta (somente apos leitura completa)
                    if (avancarAuto) { focarProximaEtiqueta(this); }
                }
            });
    ...
    for (var cidx = 0; cidx < etiquetasCentral.length; cidx++) {
        (function(inputEtiquetaCentral) {
            ...
            inputEtiquetaCentral.addEventListener('input', function() {
                ...
                atualizarProgressoPopup(valor.length);
                if (valor.length >= 35) {
                    // v2.5.0: ao ler um display completo (35 dig) no input, replicar
                    // imediatamente para toda a faixa do split (Central). Nao depender so do
                    // change/blur, que e fragil com leitor de codigo de barras.
                    // Em inputs PT (sem tr.linha-central) replicarValor e no-op seguro.
                    if (typeof replicarValor === 'function') {
                        try { replicarValor(this, 'etiqueta'); } catch (eR) {}
                    }
                    this.setAttribute('data-auto-advance', '1');
                    this.blur();
                }
            ...
            inputEtiquetaCentral.addEventListener('blur', function() {
                ocultarPopupEtiqueta();
                // v2.0.3: so avanca apos leitura completa (scanner); clique fora nao pula input.
                var avancarAuto = this.getAttribute('data-auto-advance') === '1';
                this.removeAttribute('data-auto-advance');
                var valorAtual = (this.value || '').replace(/\D/g, '');
                if (avancarAuto && valorAtual.length === 35) {
                    focarProximaEtiqueta(this);
                }
            });
...
function abrirModalInserir(botao) {
    ...
    // Sugerir o numero do posto a partir da referencia (pode ser editado).
    // Ignora codigos sinteticos de linhas manuais (ex.: "M17..."), que nao sao
    // o numero real do posto.
    var campoCodigo = document.getElementById('novo_codigo');
    if (campoCodigo) {
        var refStr = (posto || '');
        campoCodigo.value = (refStr.charAt(0) === 'M') ? '' : refStr.replace(/\D/g, '');
    }
...
(function(){
    ...
    function addBtn(){
        ...
        btn.addEventListener('click', function(){
            ...
            if(f){
                if(window.confirmarLotesJaEmOficioAntesGerar && !window.confirmarLotesJaEmOficioAntesGerar()) { return; }
